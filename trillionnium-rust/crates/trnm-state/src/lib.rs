...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PendingResolveApproval {
...
fn validate_gov_param_value(key: &str, value: &str) -> Result<(), String> {
    match key {
        ...
        "resolve_authority" => {
            ...
            if trimmed != value {
                return Err(format!(
                    "invalid governance value for {}: must be canonical (no surrounding whitespace)",
                    key
                ));
            }
            if trimmed.len() > 128 {
...
impl StateStore {
    ...
    pub fn stage_or_confirm_resolve_approval(
        ...
        match self.pending_resolve_approvals.get_mut(&task_id) {
            None => {
    ...
    pub fn state_root(&self) -> Hash32 {
        ...
        for (task_id, pending) in &self.pending_resolve_approvals {
            ...
            hasher.update([pending.approvals]);
            hasher.update(pending.first_approver.as_bytes());
            hasher.update(pending.authority_snapshot.as_bytes());
        }
...
mod tests {
    ...
    fn governance_resolve_authority_rejects_non_canonical_value_without_mutation() {
        ...
        // Reject path must not mutate active value or queue pending activation.
        assert_eq!(
            st.gov_param_string("resolve_authority"),
            Some("resolver-v1".into())
        );
