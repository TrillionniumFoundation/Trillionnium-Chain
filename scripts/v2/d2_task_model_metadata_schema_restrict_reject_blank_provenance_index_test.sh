...
cat >"$tmp_file" <<'JSON'
{
  ...
  "provenance": {
    ...
    "produced_at": "2026-03-01T03:11:00Z",
    "provenance_index": "",
...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] expected non-zero exit when restricted privacy_tier has blank provenance_index" >&2
  exit 1
fi

if ! grep -Fq "provenance.provenance_index does not match pattern ^prov:[a-z0-9:_-]{8,128}$" <<<"$output"; then
  echo "[FAIL] missing explicit provenance_index blank pattern error" >&2
  echo "$output" >&2
  exit 1
fi

echo "[PASS] D2 schema gate rejects restricted privacy tier with blank provenance_index"
