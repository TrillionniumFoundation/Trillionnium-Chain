...
for line in "${required_lines[@]}"; do
  if ! grep -Fq -- "$line" "$SCRIPT"; then
    echo "[FAIL] quick_gate_shell.sh missing deterministic locale export: $line" >&2
    exit 1
  fi
done

echo "[PASS] quick_gate_shell keeps deterministic locale exports aligned with CI shell gates"
