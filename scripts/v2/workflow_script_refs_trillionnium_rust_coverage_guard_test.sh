...
required_direct_refs=(
  'trillionnium-rust/scripts/run_agent_user_phasea_gate.sh'
  'trillionnium-rust/scripts/run_phasea_fault_injection_suite.sh'
  'trillionnium-rust/scripts/run_phasea_soak_gate.sh'
)
...
if int(data.get('script_ref_count', 0)) != 3:
    raise SystemExit(f"[FAIL] expected exactly 3 direct trillionnium-rust refs, got: {data}")
print('[PASS] workflow script ref validator covers direct trillionnium-rust/scripts references used by workflows')
PY
