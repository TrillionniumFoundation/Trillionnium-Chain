...
cat >"$tmp_file" <<'JSON'
{
  ...
  "provenance": {
    ...
    "produced_at": "2026-03-03T03:28:00Z ",
...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] expected non-zero exit for produced_at with trailing whitespace" >&2
  exit 1
fi

if ! grep -Fq "provenance.produced_at does not match pattern" <<<"$output"; then
  echo "[FAIL] missing explicit produced_at pattern rejection" >&2
  echo "$output" >&2
  exit 1
fi

echo "[PASS] D1 schema gate rejects provenance produced_at trailing whitespace"