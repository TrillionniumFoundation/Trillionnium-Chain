...
cat >"$tmp_file" <<'JSON'
{
  ...
  "provenance": {
    ...
    "produced_at": "2026-03-02T02:00:00Z",
...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] expected non-zero exit for leading-space model.model_id" >&2
  exit 1
fi
...
echo "[PASS] D1 schema gate rejects model.model_id with leading space"
