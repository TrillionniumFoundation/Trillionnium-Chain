...
CARGO_PROXY="${HOME:?HOME is required}/.cargo/bin/cargo"
...
if [[ "$(command -v cargo)" != "$CARGO_PROXY" ]]; then
  printf '%s\n' "Cargo must resolve from the hardened runner binary authority" >&2
  exit 2
fi
if ! "$CARGO_PROXY" +"$FUZZ_TOOLCHAIN" fuzz --help >/dev/null 2>&1; then
  printf '%s\n' \
    "cargo-fuzz 0.13.2 is required; see trillionnium/fuzz/README.md" >&2
  exit 2
fi
FUZZ_VERSION="$("$CARGO_PROXY" +"$FUZZ_TOOLCHAIN" fuzz --version)"
...
for target in canonical_tx_json signed_envelope_json poco_cev0_exact; do
  ...
  "$CARGO_PROXY" +"$FUZZ_TOOLCHAIN" fuzz run "$target" "$WORK_ROOT/$target" -- \
