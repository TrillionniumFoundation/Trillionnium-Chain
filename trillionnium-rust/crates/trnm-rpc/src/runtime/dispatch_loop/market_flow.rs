...
pub(super) fn handle_market_create_task(
    creator: String,
    bounty: u128,
    description: String,
) -> Result<()> {
...
pub(super) fn handle_market_submit_bid(task_id: u64, worker: String, price: u128) -> Result<()> {
...
pub(super) fn handle_market_match_task(task_id: u64) -> Result<()> {
    let tasks_path = market_tasks_file();
    let _tasks_lock = acquire_market_file_lock(&tasks_path)?;
    let bids_path = market_bids_file();
    let _bids_lock = acquire_market_file_lock(&bids_path)?;
...
pub(super) fn handle_market_report() -> Result<()> {
    let tasks = load_market_tasks();
    let bids = load_market_bids();
