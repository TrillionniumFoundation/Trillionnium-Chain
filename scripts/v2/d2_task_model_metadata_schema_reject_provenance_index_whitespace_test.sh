...
cat >"$tmp_file" <<'JSON'
{
  ...
  "provenance": {
    ...
    "produced_at": "2026-03-01T01:40:00Z",
    "provenance_index": "prov:lane dae task 20260301",
...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] expected non-zero exit for provenance_index containing whitespace" >&2
  exit 1
fi

if ! grep -Fq "provenance.provenance_index does not match pattern ^prov:[a-z0-9:_-]{8,128}$" <<<"$output"; then
  echo "[FAIL] missing explicit provenance_index whitespace pattern error" >&2
  echo "$output" >&2
  exit 1
fi

echo "[PASS] D2 schema gate rejects provenance_index containing whitespace"
