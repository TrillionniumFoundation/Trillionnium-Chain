...
for marker in "$X2_MARKER" "$I2_MARKER" "$M2_MARKER" "$MV2_MARKER" "$D2_MARKER"; do
  if [[ ! -f "$marker" ]]; then
    echo "[FAIL] expected all lane XI/MV2/D2 gates (x2/i2/m2/mv2/d2) to be invoked by run_p1_integration_gate.sh"
    cat "$TMP_DIR/out.log"
    exit 1
  fi
done

echo "[PASS] run_p1_integration_gate invokes lane XI/MV2/D2 gate chain (x2/i2/m2/mv2/d2)"
