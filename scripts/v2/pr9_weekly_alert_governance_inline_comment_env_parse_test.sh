...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/trnm-pr9-inline-comment-env.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
export WARN_UNRESOLVED_CHALLENGES="2" # previous warn threshold
export FAIL_UNRESOLVED_CHALLENGES='5' # previous fail threshold
ENV

cat >"$TMP/run/pr9/alert-thresholds.env" <<'ENV'
export WARN_UNRESOLVED_CHALLENGES="4" # raised after review
export FAIL_UNRESOLVED_CHALLENGES='5' # unchanged
ENV
...
cat >"$TMP/run/pr9/history/weekly-alert-governance-20260305T000000Z.json" <<'JSON'
{
  ...
  "history_fingerprint_sha256": "prev-inline-comment-env"
}
JSON
...
print('[OK] pr9 weekly alert governance strips shell quotes and inline comments from threshold env files')
PY
