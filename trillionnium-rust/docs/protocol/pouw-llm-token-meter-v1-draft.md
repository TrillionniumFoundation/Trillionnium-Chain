...
- CLI audit visibility: `trnm-cli query task <task_id>`, `trnm-cli query events <task_id> --limit N`, and `trnm-cli query request-full <request_id> --limit N` now pretty-print the same nested metering audit block / timeline when the underlying RPC response includes it; `query events` and `query request-full` also support `--summary` for a compact human-auditable metering timeline view that directly explains floor / bonus / rebate hits derived from the snapshotted policy
