...
mod tests {
    ...
    fn retained_wal_summary_reports_checkpoint_ahead_of_retained_tip() {
        let recovered = recovered_state(6, 2, Some(8), false);
...
"#,
    ...
    #[test]
    fn recover_metadata_only_error_reports_missing_checkpoint_metadata_when_wal_is_retained() {
    ...
    fn resolve_wal_dir_auto_isolates_existing_builtin_default_wal_meta_only_state() {
        ...
        let notice = notice.expect(
            "auto mode should surface an operator notice when wal-meta-only default WAL state triggers isolation",
        );
        assert_ne!(resolved, PathBuf::from(DEFAULT_BFT_WAL_DIR));
        assert!(resolved.starts_with(PathBuf::from(DEFAULT_BFT_WAL_DIR)));
        assert!(
    ...
    fn resolve_wal_dir_auto_isolates_existing_builtin_default_checkpoint_only_state() {
        ...
        let notice = notice.expect(
            "auto mode should surface an operator notice when checkpoint-only default WAL state triggers isolation",
        );
        assert_ne!(resolved, PathBuf::from(DEFAULT_BFT_WAL_DIR));
        assert!(resolved.starts_with(PathBuf::from(DEFAULT_BFT_WAL_DIR)));
        assert!(
    ...
    #[test]
    fn resolve_wal_dir_reuse_keeps_existing_default_state_without_notice() {
        let root = temp_wal_dir("default-wal-root-reuse");
        let base = root.join(DEFAULT_BFT_WAL_DIR);
        fs::create_dir_all(&base).unwrap();
        fs::write(checkpoint_file(&base), "existing").unwrap();
        ...
        assert!(
            notice.is_none(),
            "reuse mode should not emit an isolation notice when explicitly recovering prior WAL state"
        );
