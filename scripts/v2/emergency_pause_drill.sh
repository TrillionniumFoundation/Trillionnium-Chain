...
./scripts/legacy_node_cargo.sh run -q --features legacy-harness --bin trnm-sim -- --config configs/node1.toml --block-ms 5 --max-blocks 4 --demo-tasks 4 --demo-keys 2 --parallel-workers 4 > /tmp/trnm-node-baseline.log
...
./scripts/legacy_node_cargo.sh test -q tests::emergency_pause_rejection_formula_is_exact_boolean_gate
