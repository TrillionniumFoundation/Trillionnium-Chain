...
readonly -a NO_CARGO_EXPECTED_SUMMARIES=(
  ...
  'poco_g3_consensus_fleet_test=passed positives=24 negatives=44 parallel_process_contract=true signed_journal_required=true reduced_native_client_pre_effect_refusal=true fleet_start_certificate_required=true signed_report_required=true signed_metrics_required=true signed_final_state_required=true macos_independent_verifier_required=true sealed_replay_archive_export_required=true macos_replay_archive_verifier_required=true fault_matrix_completed=false performance_evidence=false g3_complete=false geo_wan=false production_activation=false'
  ...
  'planned_p2p_connectivity_admission_v1_test=passed source_hosts=5 endpoints=7 physical_edges=35 logical_edges=42 reduced_sources=2 reduced_edges=14 exact_inventory_placement=true canonical_bytes_preserved=true strict_frames=true double_sided_join=true bounded_retry=true icmp_green_tcp_edge_failure=blocked helper_ttl=true rebind_cleanup=true firewall_mutated=false firewall_policy_attested=false p2p_identity_metadata_bound=true p2p_identity_authenticated=false validator_binary_deployed=false validator_secret_deployed=false validator_run=false production=false g3=false fault=false performance=false geo_wan=false'
...
run_exact_python \
  "scripts/poco-fleet/check_topology.py" \
  'poco_g3_topology_planner=passed counts=7,31,100 profiles=equal,bounded-unequal five_linux_validator_hosts=true mac_observer=true all_six_hosts_participate=true unique_ports=true deterministic=true test_keys=false canonical_bytes_unchanged=true reduced_desktop4_rog3_mac=true reduced_not_full_fleet=true'
