...
- placeholder-only handoff packet 模板现已单独收口到 `trillionnium/docs/release/TRNM_EXPLORER_SCAFFOLD_HANDOFF_TEMPLATE_2026-04-04.md`
- durable-service handoff packet 的 truth-source skeleton 已补到 `trillionnium/docs/release/TRNM_DURABLE_READ_SERVICE_HANDOFF_TEMPLATE_2026-04-04.md`，用于要求 operator 同时抄出：`ingestion_source` / `checkpoint_store` / `replay_start_anchor` / `retention_scope` / `archive_owner` / `lag_slo`，以及 non-placeholder deployment / replay / restore 证据
