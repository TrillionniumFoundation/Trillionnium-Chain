...
mod tests {
    ...
    #[test]
    fn renew_capability_rejects_same_height_as_capability_revocation_without_side_effects() {
        let mut reg = IdentityRegistry::default();
        reg.register_did(
            "did:trnm:agent-renew-token-same-height".to_string(),
            "org:lane2-admin".to_string(),
            10,
        )
        .unwrap();
        ...
        let err = reg
            .renew_capability("org:lane2-admin".to_string(), token_id, 40, Some(120))
            .unwrap_err();
    ...
    #[test]
    fn renew_capability_rejects_same_height_as_did_revocation_without_side_effects() {
        let mut reg = IdentityRegistry::default();
        reg.register_did(
            "did:trnm:agent-renew-same-height".to_string(),
            "org:lane2-admin".to_string(),
            10,
        )
        .unwrap();
        ...
        reg.revoke_did(
            "org:lane2-admin".to_string(),
            "did:trnm:agent-renew-same-height",
            40,
        )
        .unwrap();
        ...
        assert!(matches!(
            err,
            InteropIdentityError::DidRevoked { did } if did == "did:trnm:agent-renew-same-height"
        ));
        assert_eq!(reg.audit_trail().len(), audit_len_before);
        assert_eq!(reg.capability(token_id).unwrap(), &token_before);
    }
