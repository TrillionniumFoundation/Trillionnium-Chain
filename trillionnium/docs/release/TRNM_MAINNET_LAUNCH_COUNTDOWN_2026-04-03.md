...
- placeholder-only 的 operator handoff 模板，能把当前 bring-up / status / rollback 证据写成不夸大 blocker closure 的 ticket 文本（`trillionnium/docs/release/TRNM_EXPLORER_SCAFFOLD_HANDOFF_TEMPLATE_2026-04-04.md`）
- 一份 **future-state** 的 durable read service handoff skeleton，已经把 non-placeholder deployment / replay / restore / lag / anchor 证据应如何冻结写清楚，但它只是模板，不是当前 durable read closure 证据（`trillionnium/docs/release/TRNM_DURABLE_READ_SERVICE_HANDOFF_TEMPLATE_2026-04-04.md`）
