...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/trnm-pr9-future-ts.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
{"at_utc":"2999-01-01T00:00:00Z","record_type":"delivery_summary","event":"partial_success","channels_ok":9,"channels_failed":9}
JSONL

cat >"$TMP/run/pr7-alert-delivery/dead-letter.jsonl" <<'JSONL'
{"created_at_utc":"2026-03-11T12:00:00Z","message":"recent"}
{"created_at_utc":"2999-01-01T12:00:00Z","message":"future"}
JSONL
...
print('[OK] pr9 weekly alert governance excludes future-dated audit and dead-letter rows from weekly metrics')
PY
