...
mod tests {
    ...
    #[test]
    fn fairness_deferral_duplicate_does_not_increment_backpressure_duplicate_metric() {
        let mut gate = AdmissionGate::new(2);
        assert_eq!(gate.admit(1), AdmitOutcome::Accepted);
        assert_eq!(gate.admit(2), AdmitOutcome::Accepted);
        ...
        // Duplicate generated by fairness-idempotency should not be counted as
        // duplicate(backpressured) noise in backpressure-specific telemetry.
        let m = gate.metrics();
        assert_eq!(m.fairness_deferrals, 1);
        assert_eq!(m.duplicates, 1);
        assert_eq!(m.backpressure_duplicates, 0);
