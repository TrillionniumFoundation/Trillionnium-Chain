...
mod tests {
    ...
    use trnm_state_sync_v0::{
        chunk_merkle_root_v0, verify_trust_path_v0, CheckpointLinkV0, CheckpointProofVerifierV0,
        Digest32V0 as SyncDigest, InstallReceiptV0, NonDestructiveInstallTargetV0, SnapshotChunkV0,
        SnapshotManifestV0, StagingIdentityV0, StateRootRecomputerV0, StateSyncSessionV0,
        WeakSubjectivityAnchorV0,
    };
    use trnm_tx_lifecycle_v0::{
        AuthorizationVerifierV0, BroadcastReceiptV0, Digest32V0 as TxDigest, FinalityWitnessV0,
        OrderedPositionV0, ProposalHandoffV0, ResourceLimitsV0, TxIntentV0, TxLifecycleErrorV0,
        TxLifecycleV0,
    };
    ...
    impl AuthorityCoordinatorV0 for DurableCoordinator {
        ...
        fn recover(&mut self) -> Result<RecoveryDispositionV0, Self::Error> {
            let journal = self
                .journal
                .lock()
                .map_err(|_| DurableCoordinatorError::Poisoned)?;
        ...
        fn apply(
            ...
            let (sequence, previous) =
                journal.current.map_or((0, NodeDigest([0; 32])), |receipt| {
                    (
                        receipt
                            .durable_sequence
                            .checked_add(1)
    ...
    fn operation() -> OperationBindingV0 {
        OperationBindingV0::derive(node_identity(), 10, 11, node_d(12), node_d(13), node_d(14))
    }
        ...
        fn begin_staging(
            ...
            Ok(StagingIdentityV0 {
                ...
                staging_digest: SyncDigest::hash(
                    ...
                    &[
                        &manifest.manifest_digest.0,
                        &self.next_generation.to_be_bytes(),
                    ],
                ),
            })
        }
