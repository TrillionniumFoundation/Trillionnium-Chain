...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/trnm-pr9-malformed-ts.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
{"at_utc":"2026-03-11 00:00:00","record_type":"delivery_summary","event":"partial_success","channels_ok":9,"channels_failed":9}
{"at_utc":"not-a-timestamp","record_type":"delivery_summary","event":"partial_success","channels_ok":8,"channels_failed":8}
JSONL

cat >"$TMP/run/pr7-alert-delivery/dead-letter.jsonl" <<'JSONL'
{"created_at_utc":"2026-03-11T12:00:00Z","message":"recent"}
{"created_at_utc":"2026-03-11 12:00:00","message":"naive"}
{"created_at_utc":"not-a-timestamp","message":"invalid"}
JSONL
...
print('[OK] pr9 weekly alert governance excludes malformed and naive timestamps from weekly metrics')
PY
