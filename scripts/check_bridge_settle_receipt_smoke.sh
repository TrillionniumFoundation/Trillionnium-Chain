...
run_step "bridge-relay: reject stale-config finalize after governance change" \
  bash -c 'cargo test --manifest-path contracts/bridge-relay/Cargo.toml --lib --tests finalize_settlement_rejects_stale_config_version_after_governance_change -- --nocapture'

run_step "bridge-relay: reject target-bridge mismatch without audit side effects" \
  bash -c 'cargo test --manifest-path contracts/bridge-relay/Cargo.toml --lib --tests finalize_settlement_rejects_target_bridge_mismatch_without_audit_side_effects -- --nocapture'

run_step "bridge-relay: keep finalize terminal by settlement id even with fresh nonce" \
  bash -c 'cargo test --manifest-path contracts/bridge-relay/Cargo.toml --lib --tests finalize_settlement_is_idempotent_by_settlement_id_even_with_new_nonce -- --nocapture'

run_step "bridge-relay: duplicate finalize stays side-effect free" \
  bash -c 'cargo test --manifest-path contracts/bridge-relay/Cargo.toml --lib --tests duplicate_finalize_is_side_effect_free -- --nocapture'

run_step "bridge-relay: rollback proof side effects when finalize hits nonce collision" \
  bash -c 'cargo test --manifest-path contracts/bridge-relay/Cargo.toml --lib --tests finalize_settlement_nonce_collision_rolls_back_proof_side_effects -- --nocapture'

run_step "bridge-relay: keep finalized settlement terminal even with fresh nonce + bad receipt" \
  bash -c 'cargo test --manifest-path contracts/bridge-relay/Cargo.toml --lib --tests duplicate_finalize_with_fresh_nonce_and_bad_receipt_still_stops_at_terminal_state -- --nocapture'
...
run_step "bridge-relay: reject stale validator rotation with stale config version" \
  bash -c 'cargo test --manifest-path contracts/bridge-relay/Cargo.toml --lib --tests stale_validator_rotation_is_fail_closed_and_side_effect_free -- --nocapture'

run_step "trnm-types: enforce receipt success for finalize helper" \
  bash -c 'cargo test --manifest-path trillionnium/Cargo.toml -p trnm-types --lib settlement_state_machine_enforces_receipt_success_for_finalization -- --nocapture'

run_step "trnm-types: reject settlement finalization with failed tx receipt" \
  bash -c 'cargo test --manifest-path trillionnium/Cargo.toml -p trnm-types --test x3_settlement_stale_pending_replay settlement_finalization_rejects_failed_tx_receipt -- --nocapture'
