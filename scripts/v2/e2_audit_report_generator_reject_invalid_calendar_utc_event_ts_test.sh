...
cat > "$LOG" <<'EOF'
2026-02-30T05:42:00Z [event] event_schema=v1 event_type=challenge challenger_delta=-4 treasury_delta=4 note="invalid calendar date should be rejected"
...
if summary.get('total_audit_events') != 1:
    print('[FAIL] expected exactly one canonical audit event after invalid calendar date filtering', file=sys.stderr)
    sys.exit(1)

if counts.get('challenge', 0) != 0:
    print('[FAIL] invalid calendar event_ts must not count as challenge event', file=sys.stderr)
    sys.exit(1)
...
print('[PASS] e2_audit_report_generator_reject_invalid_calendar_utc_event_ts_test')
PY
