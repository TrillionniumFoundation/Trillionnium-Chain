...
mod tests {
    ...
    #[test]
    fn tee_verifier_backend_not_configured_maps_to_tee_specific_unavailable_message() {
        let result = TeeVerifier::classify_backend_err(VerificationBackendError::Execution(
            BackendExecutionError::NotConfigured {
                backend: "tee:noop".to_string(),
            },
        ));
