...
- **helper 输出要原样保留**：若给 `extract_release_handoff_fields.sh` 传了票据指定的 worktree / branch，它会先调用 `verify_lane_worktree.sh` 并输出 `verified_worktree=`、`verified_branch_ref=`、`verified_head=`；这些字段应原样抄入 ticket / handoff note，作为 artifact 身份比对的 pre-run anchor。即便没有显式传 `--expected-head`，helper 也会要求 artifact 内的 `git_head=` 仍匹配当前已验证 lane HEAD；若不匹配，应按 **stale-artifact / evidence-incomplete** 处理，而不是继续引用“最新”产物。
