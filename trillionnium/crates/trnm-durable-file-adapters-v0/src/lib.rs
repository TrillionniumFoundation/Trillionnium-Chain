...
impl fmt::Display for DurableFileErrorV0 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidSnapshotManifest => f.write_str("invalid snapshot manifest for file target"),
            Self::ChunkOutOfBounds => f.write_str("snapshot chunk is outside declared bounds"),
            Self::ChunkSubstitution => f.write_str("snapshot chunk index was supplied with different bytes"),
            Self::IncompleteSnapshot => f.write_str("snapshot staging generation is incomplete"),
            Self::SnapshotByteCountMismatch => f.write_str("snapshot staged byte count differs from manifest"),
            Self::CurrentRootCasMismatch => f.write_str("current state root changed before snapshot install"),
            Self::RecoveryRequired(path) => {
                write!(f, "orphaned durable state requires explicit recovery: {}", path.display())
            }
...
impl AuthorityRecordV0 {
    ...
    fn decode(bytes: &[u8]) -> Result<Self, DurableFileErrorV0> {
        ...
        let height = u64::from_be_bytes(take_array::<8>(
            bytes,
            &mut offset,
            "missing height",
        )?);
        ...
        let stage = node_stage_from_byte(
            *bytes
                .get(offset)
                .ok_or(DurableFileErrorV0::CorruptAuthorityJournal(
...
impl FileAuthorityCoordinatorV0 {
    ...
    fn next_sequence_and_previous(
        &self,
    ) -> Result<(u64, NodeDigestV0), DurableFileErrorV0> {
        match self.current {
...
mod tests {
    ...
    fn binding(
        height: u64,
        block_id: NodeDigestV0,
        parent_id: NodeDigestV0,
    ) -> OperationBindingV0 {
    ...
    fn snapshot_target_installs_generation_with_current_root_cas() {
        ...
        {
            let mut target = AtomicSnapshotFileTargetV0::open_or_initialize(
                &directory.0,
                initial_root,
                1,
                1,
        ...
        let reopened = AtomicSnapshotFileTargetV0::open_or_initialize(
            &directory.0,
            sync_digest(99),
            99,
            99,
