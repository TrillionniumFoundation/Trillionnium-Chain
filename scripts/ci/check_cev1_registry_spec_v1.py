...
# The operation registry is a candidate projection of the closed
# ``OperationPayloadV1`` table in protocol document 08.  Keep this projection
# in the checker as an independently reviewed constant: a row that merely has
# a unique slot is not enough, because assigning a different body to a kind
# changes the bytes accepted by every downstream consumer.  ``enabled`` is
# deliberately the candidate activation bit and therefore remains false for
# every row.  The two profile-disabled rows retain their canonical slot and
# carry ``status=disabled`` plus the explicit profile error.
EXPECTED_OPERATIONS: tuple[tuple[str, str, str, str, str, str, str | None], ...] = (
    ("AgentIdentityCreation", "AgentIdentityCreationOperationBodyV1", "agent", "candidate-assigned", "existing-or-self-origin", "self-origin", None),
    ("AgentKey", "AgentKeyBodyV1", "agent", "candidate-assigned", "existing-agent", "controller", None),
    ("CapabilityGrant", "CapabilityGrantBodyV1", "agent", "candidate-assigned", "existing-agent", "controller", None),
    ("SessionKeyGrant", "SessionKeyGrantBodyV1", "agent", "candidate-assigned", "existing-agent", "controller", None),
    ("TaskCreation", "TaskCreationOperationBodyV1", "market-task", "candidate-assigned", "existing-agent", "requester", None),
    ("Bid", "BidBodyV1", "market-task", "candidate-assigned", "existing-agent", "provider", None),
    ("TaskLease", "TaskLeaseBodyV1", "market-task", "candidate-assigned", "existing-agent", "requester", None),
    ("LeaseProviderAcceptance", "LeaseProviderAcceptanceBodyV1", "market-task", "candidate-assigned", "existing-agent", "provider", None),
    ("ComputeCheckpoint", "ComputeCheckpointBodyV1", "market-task", "candidate-assigned", "existing-agent", "provider", None),
    ("ArtifactCommitment", "ArtifactCommitmentBodyV1", "data-availability", "candidate-assigned", "existing-agent", "creator", None),
    ("ExecutionReceipt", "ExecutionReceiptBodyV1", "compute-verify", "candidate-assigned", "existing-agent", "provider", None),
    ("Challenge", "ChallengeBodyV1", "compute-verify", "candidate-assigned", "existing-agent", "challenger", None),
    ("CapabilityRevocation", "CapabilityRevocationOperationV1", "agent", "candidate-assigned", "existing-agent", "controller", None),
    ("AgentAdministration", "AgentAdministrationOperationV1", "agent", "candidate-assigned", "existing-agent", "controller", None),
    ("TaskStart", "TaskStartOperationBodyV1", "market-task", "candidate-assigned", "existing-agent", "provider", None),
    ("TaskPause", "TaskPauseOperationBodyV1", "market-task", "candidate-assigned", "existing-agent", "task-party", None),
    ("TaskResume", "TaskResumeOperationBodyV1", "market-task", "candidate-assigned", "existing-agent", "provider", None),
    ("TaskCancel", "TaskCancelOperationBodyV1", "market-task", "candidate-assigned", "existing-agent", "task-party", None),
    ("TaskTimeout", "TaskTimeoutOperationBodyV1", "market-task", "candidate-assigned", "permissionless-trigger", "outer-sender", None),
    ("TaskMigration", "TaskMigrationOperationBodyV1", "market-task", "candidate-assigned", "existing-agent", "task-party", None),
    ("TaskRevision", "TaskRevisionOperationBodyV1", "market-task", "disabled", "existing-agent", "task-party", "ERR_OPERATION_DISABLED"),
    ("VerificationClaim", "VerificationClaimV1", "compute-verify", "candidate-assigned", "externally-signed-object-submitted-by-agent", "submitter", None),
    ("EvaluationResult", "EvaluationResultV1", "compute-verify", "candidate-assigned", "existing-agent", "submitter", None),
    ("ChallengeUpdate", "ChallengeUpdateOperationBodyV1", "compute-verify", "candidate-assigned", "action-dependent", "outer-sender", None),
    ("ConsumptionReceipt", "ConsumptionReceiptV1", "settlement", "candidate-assigned", "externally-signed-object-submitted-by-agent", "submitter", None),
    ("ConsumptionRollup", "ConsumptionRollupV1", "settlement", "candidate-assigned", "externally-signed-object-submitted-by-agent", "submitter", None),
    ("Settlement", "SettlementOperationBodyV1", "settlement", "candidate-assigned", "permissionless-trigger", "outer-sender", None),
    ("OrderedEvidence", "OrderedEvidenceV1", "order-coordination-settlement", "disabled", "externally-signed-object-submitted-by-agent", "submitter", "ERR_OPERATION_DISABLED"),
    ("DaObligation", "DaObligationOperationBodyV1", "data-availability", "candidate-assigned", "action-dependent", "outer-sender", None),
    ("EconomicObject", "EconomicObjectOperationBodyV1", "order-coordination-settlement", "candidate-assigned", "existing-agent", "outer-sender", None),
)
...
def check_operation_registry(document: dict[str, Any]) -> None:
    ...
    if len(rows) != len(EXPECTED_OPERATIONS):
        fail(
            "operation registry canonical projection length mismatch: "
            f"expected {len(EXPECTED_OPERATIONS)}, found {len(rows)}"
        )
    ...
    for index, row in enumerate(rows):
        ...
        allowed = required | ({"canonical_error"} if expected_error is not None else set())
        if set(row) != allowed:
        ...
        if ID_RE.fullmatch(body_type) is None:
            fail(f"{row_label}.body_type is not a canonical v1 type")
        ...
        authority = exact_string(row["authority"], f"{row_label}.authority")
        if SLUG_RE.fullmatch(authority) is None:
            fail(f"{row_label}.authority is not a canonical authority slug")
        nonce_lane = exact_string(row["nonce_lane"], f"{row_label}.nonce_lane")
        if SLUG_RE.fullmatch(nonce_lane) is None:
            fail(f"{row_label}.nonce_lane is not a canonical nonce-lane slug")
        if (
            name != expected_name
            or body_type != expected_body_type
            or plane != expected_plane
            or status != expected_status
            or authority != expected_authority
            or nonce_lane != expected_nonce_lane
            or row.get("canonical_error") != expected_error
        ):
            fail(
                f"canonical operation mapping mismatch at kind {index}: "
                f"expected name={expected_name!r}, body_type={expected_body_type!r}, "
                f"plane={expected_plane!r}, "
                f"status={expected_status!r}, authority={expected_authority!r}, "
                f"nonce_lane={expected_nonce_lane!r}, canonical_error={expected_error!r}; "
                f"found name={name!r}, body_type={body_type!r}, plane={plane!r}, status={status!r}, "
                f"authority={authority!r}, nonce_lane={nonce_lane!r}, "
                f"canonical_error={row.get('canonical_error')!r}"
            )
