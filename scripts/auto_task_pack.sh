...
TMP="$(mktemp)"
/usr/bin/python3 - "$CANDIDATES_JSON" "$CHALLENGE_JSON" "$BACKLOG_JSON" "$TASKS_FILE" "$MAX_APPEND" > "$TMP" <<'PY'
import json, sys
...
pool = list(backlog.get('candidates', [])) + list(challenge.get('candidates', [])) + list(discover.get('candidates', []))
added = []
for c in pool:
...
git add "$TASKS_FILE"
git commit -m "ops(auto-iterate): auto-fill task pool from backlog+discover+challenge" || true

echo "[task-pack] appended task lines (backlog+discover+challenge)"