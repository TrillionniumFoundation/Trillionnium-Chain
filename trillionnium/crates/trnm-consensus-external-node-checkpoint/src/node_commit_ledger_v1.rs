...
impl fmt::Display for NodeCommitLedgerErrorV1 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InvalidPath(detail) => write!(formatter, "node commit ledger path rejected: {detail}"),
            Self::InvalidState(detail) => write!(formatter, "node commit ledger state rejected: {detail}"),
            Self::SourceMismatch => formatter.write_str("node commit ledger source differs from durable head"),
            Self::TargetNotSuccessor => formatter.write_str("node commit ledger target is not the exact successor"),
            Self::ThirdState => formatter.write_str("node commit ledger observed neither exact source nor exact target"),
            Self::Io { stage, source } => write!(formatter, "node commit ledger I/O at {stage}: {source}"),
        }
    }
}
...
impl NodeCommitLedgerV1 {
    pub(crate) fn initialize_new(
        ...
        lock.try_lock_exclusive().map_err(|source| io_v1("lock ledger", source))?;
    ...
    pub(crate) fn open_existing(root: impl AsRef<Path>) -> ResultV1<Self> {
        ...
        lock.try_lock_exclusive().map_err(|source| io_v1("lock ledger", source))?;
        let anchor = decode_anchor_v1(&read_exact_file_v1(&root.join("anchor.v1"), ANCHOR_BYTES_V1)?)?;
        let anchor_digest = anchor_digest_v1(&anchor.encode_canonical());
        let mut ledger = Self {
            root,
            records,
    ...
    pub(crate) fn append_exact_successor(
        ...
        if sequence > MAX_RECORDS_V1 {
            return Err(NodeCommitLedgerErrorV1::InvalidState("record bound exceeded"));
        }
        ...
        if final_path.exists() {
            return Err(NodeCommitLedgerErrorV1::InvalidState("successor record already exists"));
        }
        let temporary = self
            .records
        ...
        self.recover_v1()?;
        if self.head.sequence != sequence || self.head.checkpoint != target || self.head.record_digest != digest {
    ...
    fn recover_v1(&mut self) -> ResultV1<()> {
        ...
        let anchor = decode_anchor_v1(&read_exact_file_v1(&self.root.join("anchor.v1"), ANCHOR_BYTES_V1)?)?;
        let anchor_digest = anchor_digest_v1(&anchor.encode_canonical());
        let (published_sequence, published_digest) = decode_head_v1(&read_exact_file_v1(&self.root.join("head.v1"), HEAD_BYTES_V1)?)?;
        if published_sequence > MAX_RECORDS_V1 {
            return Err(NodeCommitLedgerErrorV1::InvalidState("published sequence exceeds bound"));
        }
        ...
        loop {
            ...
            {
                return Err(NodeCommitLedgerErrorV1::InvalidState("record chain differs"));
            }
            record
                .target
                .validate_successor_of(&record.source)
                .map_err(|_| NodeCommitLedgerErrorV1::InvalidState("record target is not successor"))?;
            sequence = next;
        ...
        if published_sequence > sequence {
            return Err(NodeCommitLedgerErrorV1::InvalidState("published head is ahead of durable records"));
        }
        let published_actual = if published_sequence == 0 {
            anchor_digest
        } else {
            let encoded = read_exact_file_v1(&self.record_path_v1(published_sequence), RECORD_BYTES_V1)?;
            record_digest_from_encoded_v1(&encoded)?
        };
        if published_actual != published_digest {
            return Err(NodeCommitLedgerErrorV1::InvalidState("published head digest differs"));
        }
        if published_sequence < sequence {
...
fn validate_new_root_v1(path: &Path) -> ResultV1<PathBuf> {
    if !path.is_absolute()
        ...
        || path.exists()
    {
        return Err(NodeCommitLedgerErrorV1::InvalidPath("new root must be absent canonical absolute path"));
    }
    let parent = path.parent().ok_or(NodeCommitLedgerErrorV1::InvalidPath("root parent missing"))?;
    let canonical_parent = fs::canonicalize(parent).map_err(|source| io_v1("canonicalize root parent", source))?;
    if canonical_parent != parent {
        return Err(NodeCommitLedgerErrorV1::InvalidPath("root parent is not canonical"));
    }
    Ok(path.to_path_buf())
}
...
fn validate_existing_root_v1(path: &Path) -> ResultV1<PathBuf> {
    ...
    if !path.is_absolute() || canonical != path {
        return Err(NodeCommitLedgerErrorV1::InvalidPath("root is not canonical absolute path"));
    }
...
fn validate_existing_directory_v1(path: &Path, label: &'static str) -> ResultV1<PathBuf> {
    let metadata = fs::symlink_metadata(path).map_err(|source| io_v1("inspect directory", source))?;
    if !metadata.is_dir() || metadata.file_type().is_symlink() || metadata.permissions().mode() & 0o7777 != 0o700 {
...
fn validate_private_file_path_v1(path: &Path, label: &'static str) -> ResultV1<()> {
    let metadata = fs::symlink_metadata(path).map_err(|source| io_v1("inspect private file", source))?;
    if !metadata.is_file() || metadata.file_type().is_symlink() || metadata.permissions().mode() & 0o7777 != 0o600 {
        return Err(NodeCommitLedgerErrorV1::InvalidPath(label));
    }
    Ok(())
}
...
fn write_new_synced_file_v1(path: &Path, bytes: &[u8]) -> ResultV1<()> {
    ...
    file.write_all(bytes).map_err(|source| io_v1("write file", source))?;
...
fn read_exact_file_v1(path: &Path, exact: usize) -> ResultV1<Vec<u8>> {
    ...
    let metadata = file.metadata().map_err(|source| io_v1("stat ledger file", source))?;
    if metadata.len() != exact as u64 {
        return Err(NodeCommitLedgerErrorV1::InvalidState("ledger file length differs"));
    }
    let mut bytes = vec![0_u8; exact];
    file.read_exact(&mut bytes).map_err(|source| io_v1("read ledger file", source))?;
    Ok(bytes)
}
...
fn cleanup_abandoned_temps_v1(records: &Path) -> ResultV1<()> {
    for entry in fs::read_dir(records).map_err(|source| io_v1("scan records", source))? {
        ...
        if text.starts_with(".record-") && text.contains(".tmp-") {
            let metadata = entry.metadata().map_err(|source| io_v1("stat record temp", source))?;
            if !metadata.is_file() {
                return Err(NodeCommitLedgerErrorV1::InvalidState("record temp is not a file"));
            }
...
fn cleanup_abandoned_head_temp_v1(root: &Path) -> ResultV1<()> {
    for entry in fs::read_dir(root).map_err(|source| io_v1("scan ledger root", source))? {
        ...
        if text.starts_with(".head.v1.tmp-") {
            let metadata = entry.metadata().map_err(|source| io_v1("stat head temp", source))?;
            if !metadata.is_file() {
                return Err(NodeCommitLedgerErrorV1::InvalidState("head temp is not a file"));
            }
...
fn reject_unexpected_record_entries_v1(records: &Path, maximum_sequence: u64) -> ResultV1<()> {
    ...
    for entry in fs::read_dir(records).map_err(|source| io_v1("scan final records", source))? {
        ...
        let name = entry.file_name();
        let text = name.to_string_lossy();
        if !text.starts_with("record-") || !text.ends_with(".v1") || entry.file_type().map_err(|source| io_v1("stat final record", source))?.is_dir() {
            return Err(NodeCommitLedgerErrorV1::InvalidState("unexpected records directory entry"));
        }
        seen = seen.checked_add(1).ok_or(NodeCommitLedgerErrorV1::InvalidState("record count overflow"))?;
    }
    if seen != expected_count {
        return Err(NodeCommitLedgerErrorV1::InvalidState("record sequence has a gap or extra entry"));
    }
    Ok(())
}
...
fn decode_anchor_v1(raw: &[u8]) -> ResultV1<ExternalNodeCheckpointV0> {
    if raw.len() != ANCHOR_BYTES_V1 || &raw[..8] != ANCHOR_MAGIC_V1 || u16_at_v1(raw, 8)? != SCHEMA_V1 || raw[10..16].iter().any(|byte| *byte != 0) {
        return Err(NodeCommitLedgerErrorV1::InvalidState("anchor header differs"));
    }
    ...
    if digest != anchor_digest_v1(&checkpoint.encode_canonical()) {
        return Err(NodeCommitLedgerErrorV1::InvalidState("anchor digest differs"));
    }
    Ok(checkpoint)
}
...
fn decode_record_v1(raw: &[u8]) -> ResultV1<DecodedRecordV1> {
    if raw.len() != RECORD_BYTES_V1 || &raw[..8] != RECORD_MAGIC_V1 || u16_at_v1(raw, 8)? != SCHEMA_V1 || raw[10..16].iter().any(|byte| *byte != 0) {
        return Err(NodeCommitLedgerErrorV1::InvalidState("record header differs"));
    }
    let sequence = u64_at_v1(raw, 16)?;
    if sequence == 0 || sequence > MAX_RECORDS_V1 {
        return Err(NodeCommitLedgerErrorV1::InvalidState("record sequence is invalid"));
    }
    ...
    let source = ExternalNodeCheckpointV0::decode_canonical_exact(&raw[source_start..source_end])
        .map_err(|_| NodeCommitLedgerErrorV1::InvalidState("source checkpoint is not canonical"))?;
    ...
    let expected = record_digest_v1(sequence, previous_digest, &source.encode_canonical(), &target.encode_canonical());
    if record_digest != expected {
        return Err(NodeCommitLedgerErrorV1::InvalidState("record digest differs"));
    }
    Ok(DecodedRecordV1 { sequence, previous_digest, source, target, record_digest })
}
...
fn decode_head_v1(raw: &[u8]) -> ResultV1<(u64, [u8; 32])> {
    if raw.len() != HEAD_BYTES_V1 || &raw[..8] != HEAD_MAGIC_V1 || u16_at_v1(raw, 8)? != SCHEMA_V1 || raw[10..16].iter().any(|byte| *byte != 0) {
...
pub(crate) fn read_checkpoint_file_v1(path: &Path) -> ResultV1<ExternalNodeCheckpointV0> {
    ...
    file.read_to_end(&mut bytes).map_err(|source| io_v1("read checkpoint input", source))?;
    if bytes.len() != EXTERNAL_NODE_CHECKPOINT_RECORD_BYTES_V0 {
        return Err(NodeCommitLedgerErrorV1::InvalidState("checkpoint input length differs"));
    }
...
mod tests {
    ...
    fn durable_successor_reopens_exactly() {
        ...
        let mut ledger = NodeCommitLedgerV1::initialize_new(&root, source).expect("initialize ledger");
        let head = ledger.append_exact_successor(source, target).expect("append successor");
    ...
    fn journal_ahead_of_head_recovers_to_exact_target() {
        ...
        let mut reopened = NodeCommitLedgerV1::open_existing(&root).expect("recover journal-ahead cut");
        assert_eq!(
            reopened.resolve_exact_source_or_target(source, target).expect("exact convergence"),
    ...
    fn abandoned_partial_temp_converges_to_source() {
        ...
        drop(ledger);
        let mut reopened = NodeCommitLedgerV1::open_existing(&root).expect("recover partial temp cut");
        assert_eq!(
            reopened.resolve_exact_source_or_target(source, target).expect("exact convergence"),
    ...
    fn third_state_is_rejected() {
        ...
        let mut ledger = NodeCommitLedgerV1::initialize_new(&root, source).expect("initialize ledger");
        ledger.append_exact_successor(source, target).expect("first successor");
        ledger.append_exact_successor(target, third).expect("second successor");
        assert!(matches!(
