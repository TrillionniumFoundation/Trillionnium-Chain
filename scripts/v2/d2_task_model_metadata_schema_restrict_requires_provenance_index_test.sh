...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] expected non-zero exit when restricted privacy_tier omits provenance_index" >&2
  exit 1
fi

if ! grep -Fq "provenance.missing required field: provenance_index" <<<"$output"; then
  echo "[FAIL] missing explicit provenance_index required error" >&2
  echo "$output" >&2
  exit 1
fi

echo "[PASS] D2 schema gate rejects restricted privacy tier without provenance_index"
