//! Private remote-signer transport I/O with one deadline; no signer/request retry.
...
fn timeout() -> io::Error {
    io::Error::new(
        ...
        "remote signer operation deadline exceeded",
    )
}
...
mod tests {
    ...
    fn expired_budget_fails_before_socket_creation() {
        let result =
            DeadlineStream::connect(Path::new("/unavailable-remote-signer"), Instant::now());
