...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/trnm-pr9-single-quote-backslash-env.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
export ALERT_NOTIFY_CHANNEL_WARN='imessage \\ops\\primary'
ENV

cat >"$TMP/run/pr9/alert-thresholds.env" <<'ENV'
export ALERT_NOTIFY_CHANNEL_WARN='imessage \\ops\\backup'
ENV
...
cat >"$TMP/run/pr9/history/weekly-alert-governance-20260305T000000Z.json" <<'JSON'
{
  ...
  "history_fingerprint_sha256": "prev-single-quote-backslash-thresholds"
}
JSON
...
print('[OK] pr9 weekly alert governance preserves literal backslashes inside single-quoted threshold env values')
PY
