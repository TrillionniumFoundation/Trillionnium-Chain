...
# Regression 18: duplicate frozen error token in snapshot outside canonical mapping line; gate must fail on non-unique token guard.
python3 - <<'PY' "$SNAPSHOT_SPEC"
from pathlib import Path
import sys
...
spec = Path(sys.argv[1])
text = spec.read_text(encoding='utf-8')
needle = "ERR_M2V2_PROOF_INVALID"
if needle not in text:
    raise SystemExit(f"missing expected baseline phrase: {needle}")
spec.write_text(text + f"\n<!-- regression duplicate token: {needle} -->\n", encoding='utf-8')
PY

if "$GATE" >/dev/null 2>&1; then
  echo "[FAIL] MV2 gate should fail when snapshot has duplicated frozen error token outside canonical mapping line" >&2
  exit 1
fi
