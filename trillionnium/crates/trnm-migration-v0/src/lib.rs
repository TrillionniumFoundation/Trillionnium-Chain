    ...
    R: TargetRootBuilderV0,
{
    ...
    for row in &rows {
        digest_builder.update(ExportRowV0::canonical_digest(
            &row.namespace,
            &row.key,
            &row.value,
        ).0);
...
pub trait CutoverSignatureVerifierV0 {
    ...
    fn verify_attestation(
        &self,
        attestation: &CutoverAttestationV0,
...
impl fmt::Display for MigrationErrorV0 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(match self {
            ...
            Self::RowsNotStrictlyOrdered => "source export rows are not strictly ordered and unique",
...
impl<A: fmt::Display> fmt::Display for MigrationHostErrorV0<A> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::SourceFinality(error) => write!(f, "source finality verification failed: {error}"),
            Self::CutoverSignature(error) => write!(f, "cutover signature verification failed: {error}"),
        }
    }
}

impl<A> Error for MigrationHostErrorV0<A>
where
    A: Error + 'static,
{
}
...
mod tests {
    ...
    fn finalized_export_projection_and_cutover_are_exactly_bound() {
        ...
        let export_root = merkle_root_v0(&rows.iter().map(|row| row.row_digest).collect::<Vec<_>>());
        ...
        let projection = project_and_recompute_v0(
            &plan,
            &export,
            &rows,
            &IdentityProjector,
        ...
        let agreement = verify_cutover_agreement_v0(&WeightOne, &projection, 2, &attestations)
            .unwrap();
    ...
    fn signing_state_is_never_importable() {
        ...
        assert_eq!(row.validate().unwrap_err(), MigrationErrorV0::ForbiddenAuthorityState);
    }
}
