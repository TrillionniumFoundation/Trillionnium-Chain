//! Frozen-v0 native validator lifecycle without any native application boundary transport surface.
...
// Mirrors PoCO consensus v0.38's types.MaxTotalVotingPower.
...
impl ValidatorLifecycleStateV1 {
    ...
    pub fn commitment(&self) -> Result<[u8; 32]> {
        self.validate()?;
        Ok(hash_domain(
            "trnm.poco.validator-lifecycle.v1",
...
fn canonicalize_validators(
    ...
    for validator in &validators {
        let key = decode_hash32("validator public key", &validator.public_key_hex)?;
            ...
            "duplicate PoCO consensus validator address"
        );
        ensure!(
            ...
            "validator voting power exceeds PoCO consensus maximum"
        );
        total = total
            ...
            "validator total voting power exceeds PoCO consensus maximum"
        );
    }
    Ok(validators)
}
