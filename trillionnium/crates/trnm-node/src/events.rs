...
mod tests {
    ...
    use crate::{
        apply_one, balance_deltas_from_snapshot, capture_rollback_snapshot, classify_apply_error,
        compute_commitment, demo_worker_name,
        event_type_for_apply_outcome as active_event_type_for_apply_outcome,
        format_apply_event_line as active_format_apply_event_line,
        preapply_challenger_account_of as active_preapply_challenger_account_of,
        status_name as active_status_name, uses_legacy_resolve_approval_stage,
        verified_signer_of as active_verified_signer_of, EventDelta, MockTx,
    ...
    #[test]
    fn canonical_event_replay_uses_two_party_resolve_approval() {
        const TASK_ID: u64 = 1001;
        ...
        state
            ...
            .expect("canonical two-party resolve authority");
        ...
        let mut event_types = Vec::with_capacity(replay.len());
        for (offset, tx) in replay.into_iter().enumerate() {
            let tx_id = (offset + 1) as u64;
            let height = tx_id;
            let from_status = active_status_name(&state, TASK_ID);
            let challenger_account = active_preapply_challenger_account_of(&state, &tx);
            let before = capture_rollback_snapshot(&state, &tx);

            let apply_result = apply_one(&mut state, tx.clone(), height);
            let err_kind = match apply_result {
                Ok(()) => None,
                Err(error) => {
                    let kind = classify_apply_error(&error);
                    assert!(
                        uses_legacy_resolve_approval_stage(&tx, Some(kind)),
                        "unexpected replay apply error kind={kind} error={error}"
                    );
                    assert_eq!(
                        state.pending_resolve_approval(TASK_ID),
                        Some((false, 1)),
                        "first resolver must stage exactly one approval"
                    );
                    assert_eq!(
                        state.pending_resolve_first_approver(TASK_ID).as_deref(),
                        Some("authority-a")
                    );
                    assert_eq!(
                        state.get_task(TASK_ID).map(|task| task.status),
                        Some(TaskStatus::Challenged),
                        "first resolver must not advance the terminal task status"
                    );
                    for (address, balance) in &before.balances {
                        assert_eq!(
                            state.balance_of(address),
                            balance.unwrap_or(0),
                            "staged resolve must not move balance for {address}"
                        );
                    }
                    Some(kind)
                }
            ...
            let to_status = active_status_name(&state, TASK_ID);
            let root = hex::encode(state.state_root());
            let (treasury_delta, challenger_delta) = if err_kind.is_some() {
                (
                    EventDelta {
                        numeric: Some(0),
                        text: "0".into(),
                    },
                    challenger_account.as_ref().map(|_| EventDelta {
                        numeric: Some(0),
                        text: "0".into(),
                    }),
                )
            ...
            let signer = active_verified_signer_of(&state, &tx);
            let event_type = active_event_type_for_apply_outcome(&tx, err_kind);
            let line = active_format_apply_event_line(
                &state,
                &tx,
                &signer,
                tx_id,
                ...
                err_kind,
                1_000 + u128::from(tx_id),
            );
            assert!(
                line.contains(&format!("event_type={event_type}")),
                "formatted replay event lost its classified type: {line}"
            );
            println!("{line}");
        ...
        assert_eq!(state.pending_resolve_approval(TASK_ID), None);
        assert_eq!(state.pending_resolve_first_approver(TASK_ID), None);
        let task = state
            .get_task(TASK_ID)
            .expect("terminal replay task must persist");
        assert_eq!(task.status, TaskStatus::Completed);
        assert_eq!(task.challenge_bond_forfeited, Some(true));
        assert_eq!(state.balance_of(&worker), 1_000_000);
        assert_eq!(state.balance_of("challenger"), 999_990);
        assert_eq!(state.balance_of(CHALLENGE_ESCROW_ACCOUNT), 0);
        assert_eq!(state.balance_of(CHALLENGE_FORFEIT_TREASURY_ACCOUNT), 10);
        assert_eq!(state.balance_of(WORKER_SLASH_TREASURY_ACCOUNT), 0);
    }
