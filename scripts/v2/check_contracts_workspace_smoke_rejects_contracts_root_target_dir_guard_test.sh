...
BAD_TARGET_DIR="$TEST_ROOT/contracts-rust/target/contracts-rust-workspace-smoke"
set +e
...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] contracts workspace smoke accepted contracts-rust-local target dir" >&2
  exit 1
fi

if [[ "$rc" -ne 1 ]]; then
  echo "[FAIL] expected exit code 1 for contracts-rust-local target dir, got: $rc" >&2
  exit 1
fi
...
if ! grep -Fq -- "$expected" "$TMP_DIR/stderr.log"; then
  echo "[FAIL] contracts-rust-local target dir guard message not found" >&2
...
echo "[PASS] contracts workspace smoke rejects contracts-rust-local cargo target dirs"
