...
- `negative-cases.json` is the retained mutation index.  It retains the
  historical 51 cases plus three corrected-A08 operation sentinels (54 total:
  body-type drift, disabled kind-27 drift, and kind-29 `EconomicObject`
  mapping drift).  The harness copies all registry inputs into a temporary
  directory before applying each mutation; no candidate source file is
  changed.
