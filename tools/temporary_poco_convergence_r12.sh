...
FINAL_WORKTREE=/tmp/trnm-r12-final

fail_report() {
  rc=$?
  echo "r12_failure_rc=$rc"
  git status --short --untracked-files=all || true
  if [[ -d "$FINAL_WORKTREE" ]]; then
    git -C "$FINAL_WORKTREE" status --short --untracked-files=all || true
  fi
  exit "$rc"
}
...
python3 tools/temporary_poco_semantic_preservation.py
...
git commit -m "chore(poco): publish semantic-preserving native-only tree"

FINAL_COMMIT="$(git rev-parse HEAD)"
FINAL_TREE="$(git rev-parse 'HEAD^{tree}')"
printf '%s\n' "$FINAL_COMMIT" >/tmp/trnm-r12-final-commit.txt
printf '%s\n' "$FINAL_TREE" >/tmp/trnm-r12-final-tree.txt
...
python3 scripts/ci/check_native_consensus_only.py | tee /tmp/trnm-r12-native-only.json
...
python3 scripts/ci/check_external_evidence_v1.py --require-all \
  ...
  --output /tmp/trnm-r12-external-evidence.json \
  >/tmp/trnm-r12-external-evidence.stdout \
  2>/tmp/trnm-r12-external-evidence.stderr
external_rc=$?
set -e
test "$external_rc" -eq 2
grep -q "external evidence gate remains open" /tmp/trnm-r12-external-evidence.stderr
...
do
  echo "strict-clippy package=${package}" | tee -a /tmp/trnm-r12-boundary-clippy.log
  cargo clippy -p "$package" --all-targets --locked --offline -j 1 -- -D warnings \
    2>&1 | tee -a /tmp/trnm-r12-boundary-clippy.log
done
cd "$FINAL_WORKTREE"

python3 scripts/ci/check_native_consensus_only.py | tee /tmp/trnm-r12-native-only-final.json
bash scripts/ci/check_canonical_development_plan.sh
python3 scripts/ci/check_repository_truth_v1.py
python3 scripts/ci/check_required_protocol_contract_v1.py
cargo fmt --manifest-path trillionnium/Cargo.toml --all -- --check
...
trap - ERR
echo "R12_QUALIFIED_COMMIT=$FINAL_COMMIT"
echo "R12_QUALIFIED_TREE=$FINAL_TREE"
