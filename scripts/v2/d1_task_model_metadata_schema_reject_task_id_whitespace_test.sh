...
cat >"$tmp_file" <<'JSON'
{
  ...
  "provenance": {
    ...
    "produced_at": "2026-03-01T05:35:00Z",
...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] expected non-zero exit for task_id containing whitespace" >&2
  exit 1
fi
...
echo "[PASS] D1 schema gate rejects task_id containing whitespace"