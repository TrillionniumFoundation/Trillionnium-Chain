    ...
    I: IoRuntimeV0,
{
    ...
    pub fn recover(
        ...
        self.readiness = match self.coordinator.recover().map_err(HostErrorV0::Coordinator)? {
    ...
    pub fn step(
        &mut self,
    ) -> Result<HostStepV0, HostErrorV0<C::Error, I::Error>> {
        if self.readiness != HostReadinessV0::Ready {
            return Err(HostErrorV0::NotReady);
        }
        match self
            .io
            .poll_ingress(self.budget)
            .map_err(HostErrorV0::Io)?
        {
...
impl fmt::Display for BoundaryErrorV0 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(match self {
            ...
            Self::OutboundFrameOutOfBounds => {
                "outbound frame length is outside the protocol bound"
            }
...
mod tests {
    ...
    fn host_refuses_work_before_recovery() {
        ...
        let mut host = PersistentValidatorHostV0::new(coordinator, io, StepBudgetV0::default())
            .unwrap();
