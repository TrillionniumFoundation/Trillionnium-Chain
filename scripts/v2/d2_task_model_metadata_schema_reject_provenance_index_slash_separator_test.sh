...
cat >"$tmp_file" <<'JSON'
{
  ...
  "provenance": {
    ...
    "produced_at": "2026-03-03T08:09:00Z",
...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] expected non-zero exit for slash-separated provenance_index" >&2
  exit 1
fi
...
echo "[PASS] D2 schema gate rejects slash-separated provenance_index"
