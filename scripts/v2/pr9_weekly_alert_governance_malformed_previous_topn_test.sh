...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/trnm-pr9-malformed-prev-topn.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
cat >"$TMP/run/pr9/history/weekly-alert-governance-20260305T000000Z.json" <<'JSON'
{
  ...
  "topn": {
    "unresolved": ["1. keep-me", 42, null, {"bad": true}, "- move-me"],
    "forfeit": "not-a-list",
    "escrow": ["- escrow-item", ["bad"]]
  },
  "threshold": {
    "changed_keys": []
  },
  "history_fingerprint_sha256": "prev-malformed-topn"
}
JSON
...
assert wow['topn']['unresolved']['exited'] == ['keep-me', 'move-me'], wow
assert wow['topn']['forfeit'] == {'entered': [], 'exited': [], 'rank_shift': []}, wow
assert wow['topn']['escrow']['exited'] == ['escrow-item'], wow
print('[OK] pr9 weekly alert governance ignores malformed previous baseline topn rows instead of crashing')
PY
