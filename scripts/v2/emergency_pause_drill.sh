...
cargo run -q -p trnm-node --features legacy-harness --bin trnm-sim -- --config configs/node1.toml --block-ms 5 --max-blocks 4 --demo-tasks 4 --demo-keys 2 --parallel-workers 4 > /tmp/trnm-node-baseline.log
