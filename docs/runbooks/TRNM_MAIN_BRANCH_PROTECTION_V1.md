...
The administrator must first record the live `main` SHA and a change-control ticket. The evidence SHA must be that exact current `main` SHA; stale successful evidence cannot authorize a settings mutation. A race on `main`, missing successful evidence, missing acknowledgement, missing ticket, an inaccessible administration endpoint, or a mismatching readback aborts the operation.
