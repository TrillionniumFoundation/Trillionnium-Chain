...
cat >"$tmp_file" <<'JSON'
{
  ...
  "provenance": {
    ...
    "produced_at": "2026-03-01T23:59:60Z",
...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] expected non-zero exit for leap-second produced_at" >&2
  exit 1
fi

if ! grep -Fq "provenance.produced_at is not a valid date-time" <<<"$output"; then
  echo "[FAIL] missing explicit produced_at date-time error for leap-second" >&2
  echo "$output" >&2
  exit 1
fi

echo "[PASS] D1 schema gate rejects provenance produced_at leap-second timestamp"