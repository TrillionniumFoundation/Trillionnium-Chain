...
- 报告会同时给出：
  ...
  - `Baseline Report Action Summary`：对 `profiling-closeout-baseline-*.md` 候选池再压成一段摘要，输出 `baseline_closeout_report_status`、`baseline_closeout_report_action`、`baseline_closeout_report_archive_candidate_count` 与 `baseline_closeout_report_followup_command`，让 autopilot 在 closeout 报告目录堆积时也能快速判断是继续保留 latest 还是需要人工归档/刷新
