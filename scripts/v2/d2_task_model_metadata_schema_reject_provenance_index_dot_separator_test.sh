...
cat >"$tmp_file" <<'JSON'
{
  ...
  "provenance": {
    ...
    "produced_at": "2026-03-03T07:59:00Z",
...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] expected non-zero exit for dot-separated provenance_index" >&2
  exit 1
fi
...
echo "[PASS] D2 schema gate rejects dot-separated provenance_index"
