...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/trnm-pr7-gate-missing-summary.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
status=FAIL
alert_code=PR6_TEST
alert_message=delivery summary missing fallback
...
print('[MOCK_PR7] wrote audit routes without delivery_summary')
raise SystemExit(0)
EOS
chmod +x "$MOCK_PR7"
...
echo "[OK] pr7 gate falls back to route-derived status when delivery_summary is missing"
