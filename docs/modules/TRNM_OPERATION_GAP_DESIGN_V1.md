...
**Owner and input.** `trnm-consensus-core::step` owns the deterministic transition. The authenticated `Input::Vote(Vote)` carries the signed vote wire object (commissioned chain/genesis context, validator identity, epoch, height, view, certified parent/proposal digest and signature); Core derives the expected `CanonicalSignIntentV0` from its own configuration and retained state. `Input::Vote` does not carry a signer intent, validator set or watermark supplied by the caller. The intent bytes must fit `MAX_CEV0_CANONICAL_SIGN_INTENT_BYTES` and the Core retained-resource bound; the signer and intent domains are selected by the canonical intent codec.

**State and durable rows.** The owner state is `(CoreState, safety_generation, pending_intent_digest, pending_kind)`. On an admissible vote, Core emits `Effect::PersistSafetyState(SafetyStatePersistenceV0)` and no signing capability. The M03 signer journal persists the canonical intent keyed by its author/kind/epoch/height/view and predecessor revision before custody is called. Only the exact `Input::StorageAck { barrier }` for that persistence effect releases `Effect::RequestSignature`; only a matching `Input::SignatureReady { id, signature }` can release `Effect::Broadcast(OutboundMessage::Vote)`. The broadcast is reconstructed from the same canonical intent and signature; Core does not accept a caller-supplied digest/revision in the ACK.
...
There is no `Input::Timeout` variant in the reviewed Core. A peer timeout is `Input::TimeoutVote(TimeoutVote)` and is authenticated/validated as a signed timeout message; a local timer callback is `Input::LocalTimeout { epoch, view }` and carries no peer signature. `Input::TimeoutCertificate(TimeoutCertificateV0)` is a separate received certificate. Each path binds the epoch/height/view and the selected timeout context from Core state. A timeout does not require an application finality proof and cannot unlock or finalize a fork by itself.

For a locally generated timeout, the transition is `LocalTimeout -> PersistSafetyState -> StorageAck -> RequestSignature -> SignatureReady -> Broadcasted`; for a peer `TimeoutVote`, signature and context admission precede any Core transition. Each signer-journal row is keyed by the exact timeout intent identity. A stale timeout, wrong epoch/view, invalid signature or conflicting same-round intent retains the prior state. Lost ACK/signature recovery is exact source/target readback and never remints a timeout. Concurrent timeout and vote requests serialize through the Core owner; only the selected successor may issue a signature request.
...
**State and durable rows.** Before mutation reserve both verification costs (including the second Core verification). The host record stores `(tc_digest, source_view, target_view, epoch, validator_set_digest, genesis_qc_digest, safety_revision, owner_generation, state)`. After strict decode, the host delivers `Input::TimeoutCertificate(TimeoutCertificateV0)` to Core; the deterministic transition is `PersistSafetyState -> StorageAck -> TimerEffect` and invokes no signer producer. The Safety row and host record are reread before returning the timer/view effect.
