...
fn spillover_recovered_id_and_next_fresh_admit_keep_cross_class_dedup_and_queue_accounting_stable() {
    ...
    assert!(matches!(gate.pop_ready(), Some(100) | Some(101) | Some(1) | Some(999)));
    assert_eq!(gate.admit(555, IngressClass::Normal), AdmitOutcome::Accepted);
    assert_eq!(gate.queued_counts(), (2, 1, 3));
    assert_eq!(gate.admit(555, IngressClass::Critical), AdmitOutcome::Duplicate);
}
