...
impl TeeVerifier {
    ...
    fn invalid_surface_label(surface: &'static str) -> &'static str {
        match surface {
            // Invalid TEE attestations should collapse quote/report evidence and
            // claims variants to the generic `claims` contract. Unlike
            // availability or internal backend errors, invalid proofs are
            // caller-actionable validation failures, so exposing quote-vs-report
            // or evidence-vs-claims branching here only encourages
            // subtype-specific handling on a fail-closed path.
            "payload/claims"
            | "evidence/claims"
            | "quote claims"
            | "report claims"
...
mod tests {
    ...
    #[test]
    fn tee_verifier_backend_invalid_quote_report_certificate_collapse_to_generic_claims_surface(
    ) {
        let result = TeeVerifier::classify_execution_err(BackendExecutionError::InvalidProof {
            backend: "tee:mock-tee-invalid".to_string(),
            reason: "quote/report certificate signature mismatch".to_string(),
        });
        ...
        assert!(!msg.contains("invalid TEE attestation quote/report evidence:"), "message: {msg}");
        assert!(!msg.contains("invalid TEE attestation quote/report claims:"), "message: {msg}");
        assert!(!msg.contains("evidence/claims"), "message: {msg}");
        assert!(!msg.contains("payload/claims"), "message: {msg}");
        assert!(
            msg.contains("quote/report certificate signature mismatch"),
            "message: {msg}"
        );
    }
    ...
    #[test]
    fn tee_verifier_backend_internal_separator_heavy_quote_report_certificates_payload_still_prefers_combined_claims_surface(
    ) {
        let result = TeeVerifier::classify_execution_err(BackendExecutionError::Internal {
            backend: "tee:mock-tee-internal".to_string(),
            reason: "quote.report:certificates payload verifier crashed".to_string(),
        });
    ...
    #[test]
    fn tee_verifier_backend_internal_camel_case_quote_report_certificate_payload_still_prefers_combined_claims_surface(
    ) {
        let result = TeeVerifier::classify_execution_err(BackendExecutionError::Internal {
            backend: "tee:mock-tee-internal".to_string(),
            reason: "quoteReportCertificatePayload verifier crashed".to_string(),
        });
    ...
    #[test]
    fn tee_verifier_backend_internal_camel_case_quote_report_certificate_still_prefers_combined_evidence_surface(
    ) {
        let result = TeeVerifier::classify_execution_err(BackendExecutionError::Internal {
            backend: "tee:mock-tee-internal".to_string(),
            reason: "quoteReportCertificate verifier crashed".to_string(),
        });
    ...
    #[test]
    fn tee_verifier_backend_internal_camel_case_quote_report_evidence_still_prefers_combined_evidence_surface(
    ) {
        let result = TeeVerifier::classify_execution_err(BackendExecutionError::Internal {
            backend: "tee:mock-tee-internal".to_string(),
            reason: "quoteReportEvidence verifier crashed".to_string(),
        });
    ...
    #[test]
    fn tee_verifier_backend_unavailable_camel_case_quote_report_receipt_claims_maps_to_combined_claims_surface(
    ) {
        let result = TeeVerifier::classify_execution_err(BackendExecutionError::Unavailable {
            backend: "tee:mock-tee-unavailable".to_string(),
            reason: "quoteReportReceiptClaims verifier unavailable".to_string(),
        });
    ...
    #[test]
    fn tee_verifier_backend_unavailable_camel_case_quote_report_receipt_maps_to_combined_evidence_surface(
    ) {
        let result = TeeVerifier::classify_execution_err(BackendExecutionError::Unavailable {
            backend: "tee:mock-tee-unavailable".to_string(),
            reason: "quoteReportReceipt verifier unavailable".to_string(),
        });
