...
补充：`checkedAt` 目前是共享 contract 字段，语义只接受两类值——链高度标记（`height:<non-negative integer>`）或 ISO-8601 时间字符串。类型层与 zod schema 需保持同步；TypeScript 合约侧应保持为收窄后的 `HeightCheckedAt | IsoDatetimeString`，其中 `HeightCheckedAt` 至少应收窄到整数型高度标记（当前用 `` `height:${bigint}` `` 近似表达），不应把它当成任意自由格式时间文本。
