...
cat > "$LOG" <<'EOF'
2026-03-02T04:30:00Z [event] event_schema=v1 event_type=slash challenger_delta=007 treasury_delta=-0003 note="zero-padded should be ignored"
...
if impact.get('challenger_delta_total') != -4:
    print('[FAIL] challenger_delta_total should ignore zero-padded numeric token', file=sys.stderr)
    sys.exit(1)

if impact.get('treasury_delta_total') != 4:
    print('[FAIL] treasury_delta_total should ignore zero-padded numeric token', file=sys.stderr)
    sys.exit(1)

print('[PASS] e2_audit_report_generator_ignore_zero_padded_numeric_delta_test')
PY
