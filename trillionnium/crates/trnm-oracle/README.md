...
Oracle report validation, policy enforcement, observation formatting, and snapshot helpers.

## Responsibilities

- Validate bounded oracle observations against configured source, sample, age, drift, and formatting policy.
- Produce deterministic validation/observation reports and snapshots for adjacent read/settlement paths.
- Provide negative tests for malformed or policy-violating reports.

## Non-responsibilities and production boundary

- Oracle is outside the frozen Day-1 canonical scope.
- Validation of a report does not prove source honesty, transport authenticity, or economic correctness.
- This crate does not itself authorize settlement or mutate canonical balances.
...
- `src/lib.rs`: report model, validation, formatting, and public API.
- `src/validation/policy.rs`: policy checks.
- `src/snapshot.rs`: snapshot representation.
- `src/tests/`: policy, validation, formatting, and snapshot tests.

## Required invariants

- Source identity, observation time/height, units/scale, sample count, and signed payload are unambiguous.
- Stale, insufficient, out-of-range, duplicate, or unknown-source observations fail closed.
- Aggregation and rounding are deterministic and overflow safe.
- Policy version is bound to every accepted report and downstream decision.
...
```bash
cargo check -p trnm-oracle --locked
cargo test -p trnm-oracle --locked
```

Additional evidence:

- `cargo test -p trnm-oracle --locked`
- Activation requires source-failure, stale-data, disagreement, manipulation, and recovery drills.
...
- Specify source authentication, aggregation/median rules, dispute/slashing economics, fallback, pause, and governance.
- Integrate only through typed canonical transactions with multi-validator evidence.
