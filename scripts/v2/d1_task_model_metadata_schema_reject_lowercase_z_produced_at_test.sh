...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] expected non-zero exit for lowercase z timezone suffix" >&2
  exit 1
fi

if ! grep -Fq "provenance.produced_at is not a valid date-time" <<<"$output"; then
  echo "[FAIL] missing explicit produced_at date-time format error" >&2
  echo "$output" >&2
  exit 1
fi

echo "[PASS] D1 schema gate rejects lowercase z timezone suffix"
