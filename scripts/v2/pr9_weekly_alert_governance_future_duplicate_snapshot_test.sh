...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/trnm-pr9-future-dup-snapshot.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
{
  "history_fingerprint_sha256": "future-stray"
}
JSON
...
if [[ "$third_count" != "3" ]]; then
  echo "[FAIL] expected future-dated stray snapshot not to trigger another duplicate snapshot, got $third_count"
...
echo "[OK] pr9 duplicate snapshot suppression ignores future-dated history snapshots"
