...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] expected non-zero exit for trailing-space provenance.producer_did" >&2
  exit 1
fi
...
echo "[PASS] D1 schema gate rejects provenance.producer_did with trailing space"