...
cat >"$tmp_file" <<'JSON'
{
  ...
  "provenance": {
    ...
    "provenance_index": "prov:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] expected non-zero exit for provenance_index overflow" >&2
  exit 1
fi
...
echo "[PASS] D2 schema gate rejects provenance_index overflow"
