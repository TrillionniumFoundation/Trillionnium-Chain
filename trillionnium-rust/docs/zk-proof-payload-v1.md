...
- `backend_id`, when present, must be a non-empty canonical token without surrounding whitespace, must remain a single opaque token without embedded whitespace or control characters (including Unicode whitespace), and must contain at least one visible canonical backend token segment rather than punctuation-only noise; the legacy `noop` selector remains the only supported explicit no-backend alias.
