...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/trnm-pr9-malformed-prev-thresholds.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
FAIL_UNRESOLVED_CHALLENGES=5
WARN_FORFEITS_DAILY_INCREASE=3
ENV
...
cat >"$TMP/run/pr9/history/weekly-alert-governance-20260305T000000Z.json" <<'JSON'
{
  ...
  "threshold": {
    "changed_keys": "not-a-list"
  },
  "history_fingerprint_sha256": "prev-malformed-thresholds"
}
JSON
...
assert wow['threshold_changed_keys_delta'] == 2, wow
assert wow['threshold_new_keys_vs_last_week'] == [
    'WARN_FORFEITS_DAILY_INCREASE',
    'WARN_UNRESOLVED_CHALLENGES',
], wow
assert wow['threshold_removed_keys_vs_last_week'] == [], wow
print('[OK] pr9 weekly alert governance ignores malformed previous threshold changed_keys payloads')
PY
