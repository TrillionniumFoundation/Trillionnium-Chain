...
fn settle_worker_stake_for_terminal_state(
    ...
    // Fail closed before any terminal worker-stake movement when challenged-task
    // evidence/settlement metadata is malformed or incomplete. Keep the scope
    // narrow so uncontested timeout-completion paths do not inherit challenge-only
...
mod tests {
    ...
    #[test]
    fn slashed_challenge_terminal_state_requires_retained_evidence_surface() {
        let mut st = seeded_state();
        st.set_balance("challenger", 100);

        let r1 = apply_create_task(&mut st, 778, "alice".into(), 10).unwrap();
        let result_hash = [3u8; 32];
        let reveal_salt = [4u8; 32];
        let committed = compute_commitment(778, &result_hash, &reveal_salt, "worker1");
    ...
    #[test]
    fn slashed_terminal_state_rejects_forfeited_challenge_outcome_metadata() {
        let mut st = seeded_state();
        st.set_balance("challenger", 100);

        let r1 = apply_create_task(&mut st, 7781, "alice".into(), 10).unwrap();
        let result_hash = [8u8; 32];
        let reveal_salt = [9u8; 32];
        let committed = compute_commitment(7781, &result_hash, &reveal_salt, "worker1");
    ...
    fn timeout_rejects_terminal_non_challenged_task_with_stale_challenge_timing_metadata() {
        ...
        let err = apply_timeout(&mut st, bad_ref, 212).unwrap_err();
        assert!(matches!(err, PouwError::State(msg) if msg.contains("stale challenge metadata")));
    }
    ...
    #[test]
    fn slashed_terminal_settlement_rejects_blank_challenger_metadata_without_moving_balances() {
        let mut st = seeded_state();
        st.set_balance("challenger", 100);
        st.set_balance("worker1", 50);

        let r1 = apply_create_task(&mut st, 40_103_2, "alice".into(), 10).unwrap();
        let result_hash = [4u8; 32];
        let reveal_salt = [8u8; 32];
        let committed = compute_commitment(40_103_2, &result_hash, &reveal_salt, "worker1");
        let r2 = apply_accept_task(&mut st, r1, "worker1".into()).unwrap();
        let r3 =
            apply_commit_result_at_height(&mut st, r2, "worker1".into(), committed, 100).unwrap();
        let r4 = apply_reveal_result_at_height(&mut st, r3, result_hash, reveal_salt, None, 110)
