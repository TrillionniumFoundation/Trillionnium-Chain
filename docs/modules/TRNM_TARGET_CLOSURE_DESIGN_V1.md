...
* **Crash contract.** Before native P, a restart may call
  ...
  The pre-P readback remains comparison-only. The post-P/D/C path now has an
  authenticated resumed Vote replay: `recover_progressed_continuing_v1`
  rebinds a private Core with its persisted signature gate, joins the native P
  and all custody/checkpoint owners, persists the signer intent before calling
  the producer, and persists/readbacks the Safety release before returning one
  Vote broadcast. Finality collection, a second complete epoch and the
  repeated-crossing campaign remain `open`; the existing first-new guard only
