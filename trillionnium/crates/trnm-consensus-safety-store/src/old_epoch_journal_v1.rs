...
impl OldEpochSafetyJournalProfileV1 {
    ...
    fn context(&self) -> Result<SafetyStateRecordContextV0<'_>> {
...
impl ConfirmedOldEpochSafetyHeadV1 {
    pub const fn state_v1(&self) -> &SafetyState {
        self.record.state()
    }
...
impl SqliteOldEpochSafetyJournalV1 {
    ...
    pub fn initialize_from_journal7_v1(
        ...
        fs_owner::configure_connection(
            &connection,
            true,
            profile.limits.maximum_record_bytes().max(MAX_CONTEXT),
            profile.max_db,
        )?;
        let mut store = Self {
    ...
    pub fn fresh_read_v1(
        ...
        fs_owner::configure_connection(
            &connection,
            false,
            self.profile.limits.maximum_record_bytes().max(MAX_CONTEXT),
            self.profile.max_db,
        )?;
