...
The in-memory codec, SQLite startup/schema-migration loader, and external application adapter snapshot
...
not an uncommitted local setting. Startup, repeated `InitChain`, and external application adapter
...
external application adapter, or authenticated cross-epoch prune closure.
...
## B2-H3b2b2 application-authenticated candidate boundary (bounded shared and external application adapter/restart evidence landed; remaining closure in progress)
...
receipts and `BlockHeader::id()`. It does not equate the external application adapter/foreign header hash
...
to external application adapter startup or the production host, covers checkpoints only rather than
...
3. The specification now freezes `Valid`, retryable `Unavailable`, and
   ...
   host/external application adapter runtime-view adapter. Core privately freezes the exact positive-height
   ...
   host/external application adapter runtime view remains unwired. Legacy `load_object` has retained its
   old direct-read behavior, and the external application adapter outcome policy does not consume the
   ...
   provides `AuthorizedNativeCheckpointExecutionV0`, checkpoint, or external application adapter authority. A
   ...
   application/persistence/head update, and host outbox/Core/external application adapter callback
   ...
   `Unavailable`, or an invariant. Schema v8 adds the app-private
   process-local delivery/ack transition chain: the first seal retains a live
   non-cloneable owner, a non-cloneable driver fixes its store, owned Core, and
   injected sink, and the driver exercises the real Core persistence barrier
   ...
   production AppCore/external application adapter effect driver, general obligation/result replay, or
   ...
   `reserved`; schema v6 accepts only that state and an empty outbox.
   ...
   precedes capacity rejection. Empty v5 journals migrate atomically through
   reserved-only v6; any non-empty v5 reservation table lacks a revalidatable
   raw request record and makes migration roll back with schema and rows intact.

   Application-store schema v7 preserves valid reserved rows and activates only
   complete mixed-body computed state-root and receipts-root mismatches. Their
   fixed canonical artifact binds route/full ID, request fingerprint, immutable
   checksum, deterministic-invalid result, and closed reason. A fixed callback
   payload, idempotency key, and outbox checksum bind the same identity/result/
   artifact under four distinct hash domains. One `BEGIN IMMEDIATE` transaction
   moves `reserved` directly to `callback_pending`, stores the artifact, inserts
   exactly one congruent outbox row, and updates accounting; failed pre-commit
   attempts leave the reserved row with no outbox, while exact retry does not
   double-account. V7 rejects `Valid`, all other invalid reasons,
   `Unavailable`, `InvariantFault`, and the `evaluated`, `delivered`, `acked`,
   and `applied` states. V6-to-v7 activation verifies every reserved row,
   accounting, checksum, and the empty outbox before changing metadata and
   rolls back atomically on drift. Snapshot export
   ...
   This closes the revalidatable raw request-record/recovery-fact foundation and
   the narrow deterministic-root-mismatch artifact/outbox transaction only; it
   does not reconstruct the signed proposal witness, persist a `Valid` artifact,
   restore a Core obligation, grant executable crash takeover, apply state, or
   provide process-wide callback exactly-once. The v7 transaction itself does
   ...
   attempt at least one and no accepted-Core fields. `Acked` deletes the outbox,
   retires its accounting, and binds both an accepted Core revision later than job
   ...
   fresh execution, generic result recovery, host/AppCore/external application adapter network wiring,
   ...
   Core callback, persistence, or external application adapter authority.
   ...
   and external application adapter wiring remain hard gaps before the general terminal/Core callback.
   ...
   uses an app-private fixed-store/owned-Core/injected-test-sink driver for real
   Core delivery and acknowledgement, but supplies neither production
   SafetyState persistence nor recovery authority. A consuming dispatcher
   ...
   boundary priority evidence. Shared vectors remain unchanged.
   `PruneRetiredMeter` now has a separate prepared decrement/delete path.
   Canonical ID and exact slot precede meter `-1`, all remaining family caps,
   and the defensive total; unsupported fields then precede full policy,
   retired/strict-retention, active-certificate, retained-usage,
   authenticated kind-5 predecessor, and signed exact-delete checks. The
   active-certificate scan rebinds certificate/consumer/key/provider/task/
   meter/settlement/consumed-units/evidence fields to the authenticated kind-1
   payload,
   so owner drift is `AuthenticatedOverlay` and cannot hide a live meter
   reference. The carrier freezes the complete policy row/slot, field and raw
   semantic owners, and exact source bytes; carrier/source CAS runs before the
   exact row and kind-5 deletion. No new proof or accumulator increment is
   authorized because the permanent family-12 meter identity from definition
   remains occupied. A real H1 define-four, H2 retire-one, isolated H284
   fixture proves authenticated sorted `4 -> 3`, three untouched rows, exact
   semantic deletion, unchanged accumulator, and seal. A separate real
   accepted-certificate branch proves the active-reference rejection; one
   synthetic six-row `6 -> 5` cap collision remains handler-boundary evidence
   only.
   Shared meter-prune vectors remain byte-identical. This closes the isolated
   handler/audit surface, not production cross-epoch reachability, Core
   activation, host integration, or a phase. Every other audit-open family
   still requires its own capacity-order audit before terminal failure
   mapping.
   ...
   closure now also includes `PruneRevokedValidatorHistory`. Canonical validator
   ID and exact history slot precede history `-1`, all other family caps, and
   the defensive total. No-proof field admission then precedes complete
   history/head/revocation provenance, revoked and strict-retention checks,
   authenticated active-certificate owner/reference exclusion, an independent
   exact revoked kind-9 companion, and the signed single-delete set. The
   prepared carrier freezes the complete row/slot, both proof-field owners, raw
   semantic owner, and exact source bytes before row/leaf deletion. Permanent
   validator identity and consensus-key nullifiers remain occupied; no proof or
   accumulator increment is authorized. A real H1 four-registration/H2 revoke
   source rejects exact-boundary H282 and seals first-after H283 as sorted
   authenticated `4 -> 3`, preserving three byte-exact rows and deleting one
   kind-9 leaf. The active-reference/owner collision carries a real certificate
   body but is handler-boundary only because revoked-provider/active-certificate
   coexistence is not authenticated-reachable; the synthetic six-row `6 -> 5`
   cap collision has the same evidence limit. Frozen shared validator-prune
   bytes stay unchanged and prove isolated register/revoke/wait/prune plus the
   permanent-identity resurrection guard, not this full-capacity carrier matrix
   or production epoch reachability. This does not close Core activation, host
   integration, terminal mapping, durable-node work, or a phase. Capacity-order
   closure now also includes `PruneExpiredCertificate`. Canonical certificate
   ID and exact active-row presence precede active-certificate `-1`, every
   other family cap, and the defensive total. Unsupported-field admission then
   precedes exact certificate-body ownership and an independently derived
   five-leaf retained set: kind-1 certificate, kind-4 tuple, kind-6 consumed
   settlement, kind-7 measurement, and kind-12 pending-aware lifecycle. The
   stored acceptance-time boundary, pending/reservation guards, exact signed
   five deletes, and checked accumulator `+2` follow before clone. Its carrier
   freezes the complete row/slot, both proof-field owners, raw semantic owner,
   two permanent subjects, and all five source bytes. Carrier/source drift
   fails before the two chained proofs; row and leaves are removed only after
   both verify. A real H1 sixteen-operation/H2 four-accept chain rejects exact-
   boundary H282 and seals first-after H283 as sorted authenticated `4 -> 3`,
   preserving three byte-exact rows and unrelated nonce/usage owners while
   deleting five leaves and advancing count by two. A real H3 challenge remains
   pending beyond retention and rejects with its `ChallengePending` companion
   intact. Same-ID reservation coexistence and synthetic six-row `6 -> 5` cap
   collisions are handler-boundary witnesses only. The shared certificate-
   prune replay remains the rejected-lifecycle single-row witness, not this
   full-capacity/carrier matrix. The prune handler rebinds the retained body/
   five-leaf owners, not every acceptance-era consumer, meter, provider,
   ...
   plan application/persistence/head update, host/Core/external application adapter delivery,
   ...
   callback delivery authority, `Core::step`, Finalize, external application adapter, or phase closure.
   ...
   does not itself call a Core instance, deliver a callback, or enter external application adapter.
   ...
   existing state, scans recovery work, and excludes both job/outbox tables
   from snapshots. Schema v7 preserves those rows and adds a private consuming
   bridge plus one atomic artifact/outbox transaction for complete-body
   ...
   external application adapter, or terminal authority consumes this seam. Historical cutoff/projection
   ...
   current external application adapter execution path still erases its errors, `ProcessProposal` has
...
4. The exact inert `NextEpochCommitmentV0` object and same-version preimage
   ...
   exact checkpoint records, but it is not wired into startup/external application adapter/the host,
