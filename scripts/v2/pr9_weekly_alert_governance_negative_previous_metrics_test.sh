...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/trnm-pr9-negative-prev-metrics.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
cat >"$TMP/run/pr9/history/weekly-alert-governance-20260305T000000Z.json" <<'JSON'
{
  ...
  "history_fingerprint_sha256": "prev-negative-metrics"
}
JSON
...
print('[OK] pr9 weekly alert governance clamps negative previous baseline metrics to zero')
PY
