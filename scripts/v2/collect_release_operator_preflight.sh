...
Prints a fail-closed native-consensus release/operator preflight record for the current lane worktree.
USAGE
}
...
case "$WORKSPACE_ROOT" in
  "$ROOT"|"$ROOT"/*) ;;
  *)
    echo "[FAIL] workspace root escapes worktree root: workspace_root=$WORKSPACE_ROOT worktree_root=$ROOT" >&2
    exit 1
    ;;
esac
...
BINARY_PATH="${BINARY_PATH:-$WORKSPACE_ROOT/target/debug/trnm-chain-node}"
BINARY_BUILD_COMMAND="${BINARY_BUILD_COMMAND:-cargo build -p trnm-node --features self-consensus --bin trnm-chain-node}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --started-at-utc) STARTED_AT_UTC="${2:-}"; shift 2 ;;
    --binary-path) BINARY_PATH="${2:-}"; shift 2 ;;
    --binary-build-command) BINARY_BUILD_COMMAND="${2:-}"; shift 2 ;;
    --cli-binary-path) CLI_BINARY_PATH="${2:-}"; shift 2 ;;
    --cli-build-command) CLI_BUILD_COMMAND="${2:-}"; shift 2 ;;
    --config-set-id) CONFIG_SET_ID="${2:-}"; shift 2 ;;
    ...
    --p2p-allowlist-source) P2P_ALLOWLIST_SOURCE="${2:-}"; shift 2 ;;
    --previous-stable-anchor) PREVIOUS_STABLE_ANCHOR="${2:-}"; shift 2 ;;
    --rollback-entrypoint) ROLLBACK_ENTRYPOINT="${2:-}"; shift 2 ;;
...
if [[ -n "$EXPECTED_WORKTREE_ROOT" || -n "$EXPECTED_BRANCH" || -n "$EXPECTED_BRANCH_REF" || -n "$EXPECTED_HEAD" ]]; then
  VERIFY_ARGS=()
  [[ -z "$EXPECTED_WORKTREE_ROOT" ]] || VERIFY_ARGS+=(--expected-worktree-root "$EXPECTED_WORKTREE_ROOT")
  ...
  [[ -z "$EXPECTED_HEAD" ]] || VERIFY_ARGS+=(--expected-head "$EXPECTED_HEAD")
