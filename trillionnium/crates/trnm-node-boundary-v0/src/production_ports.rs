...
pub trait HostAttestationVerifierV0 {
    ...
    fn verify_host_attestation(&self, claim: &HostAttestationClaimV0) -> Result<(), Self::Error>;
}
        ...
        Some(previous) => {
            ...
            let expected_sequence =
                previous
                    .sequence
                    .checked_add(1)
                    .ok_or(AnchorAdvanceErrorV0::Protocol(
...
{
    let request = request.validate().map_err(RemoteSigningErrorV0::Protocol)?;
...
pub trait PersistentPacemakerPortV0 {
    ...
    fn poll_pacemaker(&mut self, identity: NodeIdentityV0) -> Result<PacemakerPollV0, Self::Error>;
    ...
    P: PersistentPacemakerPortV0,
{
    ...
    pub fn verify_startup(
        ...
        let attestation =
            verify_host_attestation_v0(&self.host_attestation, claim, identity, current_height)
...
impl fmt::Display for ProductionPortErrorV0 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(match self {
            ...
            Self::SignRequestAnchorMismatch => {
                "remote sign request is not bound to the verified anchor"
            }
            Self::SignatureReceiptMismatch => {
                "remote signature receipt is malformed or substituted"
            }
