...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/trnm-pr9-double-quote-hash-env.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
export ALERT_NOTIFY_CHANNEL_WARN="imessage ops#primary" # previous routed label
ENV

cat >"$TMP/run/pr9/alert-thresholds.env" <<'ENV'
export ALERT_NOTIFY_CHANNEL_WARN="imessage ops#backup" # current routed label
ENV
...
cat >"$TMP/run/pr9/history/weekly-alert-governance-20260305T000000Z.json" <<'JSON'
{
  ...
  "history_fingerprint_sha256": "prev-double-quote-hash-thresholds"
}
JSON
...
assert changed == [{
    ...
    'old': 'imessage ops#primary',
    'new': 'imessage ops#backup',
}], changed
assert payload['week_over_week']['threshold_changed_keys_delta'] == 1, payload['week_over_week']
print('[OK] pr9 weekly alert governance preserves hash characters inside double-quoted threshold env values')
PY
