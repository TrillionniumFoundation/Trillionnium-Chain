...
| --- | --- | --- | --- |
| Fast preflight | `run/preflight/go-no-go-latest.txt` | `result=`, `generated_at=`, `git_status_summary=`, `git_worktree_path=`, `git_worktree_branch_ref=`, `git_worktree_branch_ref_match=`, `expected_worktree_root=`, `expected_branch_ref=`, referenced log paths | Did the local rehearsal fail fast on obvious safety blockers *and* stay bound to the ticket-assigned worktree/ref? |
