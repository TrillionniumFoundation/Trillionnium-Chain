...
cat >"$tmp_file" <<'JSON'
{
  ...
  "provenance": {
    ...
    "produced_at": "2026-03-02T13:36:00Z",
    "provenance_index": "",
...
if METADATA_FILE="$tmp_file" "$GATE" >/dev/null 2>&1; then
  echo "[FAIL] D3 schema gate unexpectedly accepted blank provenance_index for restricted tier" >&2
  exit 1
fi

echo "[PASS] D3 schema gate rejects blank provenance_index for restricted privacy tier"
