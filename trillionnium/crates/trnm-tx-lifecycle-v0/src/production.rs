    ...
    V: AuthorizationVerifierV0,
{
        ...
        J: DurableTxJournalV0,
    {
        ...
        let (lifecycle, durable) = TxLifecycleV0::restore_recovered_v0(
            chain_id,
            verifier,
            recovered,
        )
        ...
        J: DurableTxJournalV0,
    {
        ...
        let record_digest = self
            ...
            .record_digest;
        let permit = verify_core_safety_permit_v0(
            permit_verifier,
            claim,
            tx_id,
            record_digest,
    ...
    where
        ...
        J: DurableTxJournalV0,
    {
        self.require_live().map_err(TxFinalizationErrorV0::Protocol)?;
        let claim = source
        ...
        J: DurableTxJournalV0,
    {
        ...
        let collected = self
            ...
            .map_err(|error| TxCollectErrorV0::Protocol(error.into()))?;
        let deletion_digest = match journal.delete_collected(
            tx_id,
...
fn validate_recovered_record_v0(
    ...
    if !valid_shape
        ...
        || record
            .broadcast_receipt
            .is_some_and(|receipt| record.broadcast_intent.is_none_or(|intent| {
...
fn tombstone_digest_v0(value: TombstoneReasonV0) -> Digest32V0 {
    match value {
        ...
        TombstoneReasonV0::Expired => {
            Digest32V0::hash(b"trnm.tx.record.tombstone.expired.v0", &[])
        }
...
impl fmt::Display for ProductionTxErrorV0 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Lifecycle(error) => write!(f, "transaction lifecycle rejected operation: {error}"),
        }
    }
}
...
mod tests {
    ...
    fn admission_is_durable_before_ack_and_restart_recovers_exact_state() {
        ...
        assert_eq!(
            coordinator.lifecycle().unwrap().record(receipt.tx_id).unwrap().phase,
            TxPhaseV0::WalPersisted
        );
        let recovered = ProductionTxCoordinatorV0::recover(
            d(1),
            AcceptAuthorization,
            &mut journal,
        )
        .unwrap();
        assert_eq!(
            recovered.lifecycle().unwrap().record(receipt.tx_id).unwrap().phase,
            TxPhaseV0::WalPersisted
        );
    }
    ...
    fn sign_broadcast_finalize_and_gc_are_durably_ordered() {
        ...
        let readback = coordinator
            .apply_finalized_readback(
                &mut MemoryFinality { claim },
                &mut journal,
                admission.tx_id,
            )
            .unwrap();
