...
pub(crate) fn predecessor() -> trnm_consensus_crypto::StrictSameVersionEpochActivationAuthorityV0 {
...
pub(crate) fn signing_key(validator: &Validator) -> SigningKey {
...
pub(crate) fn qc_with_signature_mutation(
    set: &ValidatorSet,
    header: &BlockHeader,
    corrupt: bool,
) -> QuorumCertificate {
...
pub(crate) enum SuccessorBadSignature {
    None,
    ParentQc,
    CheckpointQc,
    Seal1Qc,
...
pub(crate) fn evidence_binding(e: &EpochActivationEvidenceBytesV0) -> [u8; 32] {
...
pub(crate) fn successor_evidence(
...
pub(crate) fn successor_evidence_variant(
...
pub(crate) fn successor_evidence_with_ancestry_variant(
    context: &StrictEpochRuntimeContextV1,
    with_timeout: bool,
    bad: SuccessorBadSignature,
...
/// Inert, test-only material for the M02 retained-provenance consumer.
///
/// The roots and bindings are comparison pins.  They do not expose a signing
/// key, a strict authority, or an activation capability.
pub struct GenuineSuccessorFixtureV1 {
    pub root_validator_set: ValidatorSet,
    pub root_parameters: ConsensusParametersV0,
    pub root_binding: [u8; 32],
    pub original_roots: [Vec<u8>; 8],
    pub second_roots: [Vec<u8>; 8],
    pub terminal_binding: [u8; 32],
    pub canonical_ancestry: Vec<BlockHeader>,
}

fn authority_evidence_v1(
    authority: &trnm_consensus_crypto::StrictSameVersionEpochActivationAuthorityV0,
) -> EpochActivationEvidenceBytesV0 {
    EpochActivationEvidenceBytesV0 {
        old_checkpoint_finality: authority
            ...
            .try_cev0_bytes()
            .unwrap(),
        next_epoch_commitment: authority.next_epoch_commitment().try_cev0_bytes().unwrap(),
        authorization_kernel: authority.authorization_kernel().try_cev0_bytes().unwrap(),
        old_validator_set: authority.old_validator_set().try_cev0_bytes().unwrap(),
        old_consensus_parameters: authority.old_consensus_parameters().canonical_bytes(),
        new_validator_set: authority.new_validator_set().try_cev0_bytes().unwrap(),
...
fn evidence_roots_v1(evidence: &EpochActivationEvidenceBytesV0) -> [Vec<u8>; 8] {
    [
        evidence.old_checkpoint_finality.clone(),
        evidence.next_epoch_commitment.clone(),
        evidence.authorization_kernel.clone(),
        evidence.old_validator_set.clone(),
        evidence.old_consensus_parameters.clone(),
        evidence.new_validator_set.clone(),
        evidence.new_consensus_parameters.clone(),
        evidence.authenticated_checkpoint_parent_header.clone(),
    ]
}

/// Build the genuine mixed-anchor successor evidence used by the repeated
/// C28/S30 → C38/S40 provenance consumer tests.
pub fn genuine_successor_fixture_v1(
    predecessor_context: &StrictEpochRuntimeContextV1,
    with_mixed_anchor_timeouts: bool,
) -> GenuineSuccessorFixtureV1 {
    let original_evidence = authority_evidence_v1(predecessor_context.activation());
    let (second_evidence, _second_old_set, _second_old_parameters, second_binding, ancestry) =
        successor_evidence_variant(
            predecessor_context,
            with_mixed_anchor_timeouts,
            SuccessorBadSignature::None,
        );
    GenuineSuccessorFixtureV1 {
        root_validator_set: predecessor_context.activation().old_validator_set().clone(),
        root_parameters: *predecessor_context.activation().old_consensus_parameters(),
        root_binding: *predecessor_context.activation().binding_ref().as_bytes(),
        original_roots: evidence_roots_v1(&original_evidence),
        second_roots: evidence_roots_v1(&second_evidence),
        terminal_binding: second_binding,
