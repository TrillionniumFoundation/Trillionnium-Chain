...
cat >"$tmp_file" <<JSON
{
  ...
  "provenance": {
    ...
    "produced_at": "2026-03-01T11:00:00Z",
    "provenance_index": "prov:lane-dae:task-20260301-0128",
...
if ! grep -Fq "[PASS] D1 task/model metadata schema gate" <<<"$output"; then
  echo "[FAIL] expected schema gate pass at task_id maxLength boundary" >&2
  echo "$output" >&2
  exit 1
fi

echo "[PASS] D1 schema gate accepts task_id maxLength=128 boundary"
