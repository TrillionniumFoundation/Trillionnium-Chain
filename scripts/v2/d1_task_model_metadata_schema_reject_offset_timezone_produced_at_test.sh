...
cat >"$tmp_file" <<'JSON'
{
  ...
  "provenance": {
    ...
    "produced_at": "2026-03-02T09:10:11+00:00",
...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] expected non-zero exit for timezone offset produced_at" >&2
  exit 1
fi

if ! grep -Fq "provenance.produced_at does not match pattern" <<<"$output"; then
  echo "[FAIL] missing explicit produced_at pattern error for timezone offset" >&2
  echo "$output" >&2
  exit 1
fi

echo "[PASS] D1 schema gate rejects offset timezone produced_at"
