...
def mutate(sandbox: Path, mutation: str) -> None:
    ...
    # Operation rows are a closed projection of document 08.  Keep semantic
    # mutations separate from object/catalog recipes so every field that can
    # alter accepted operation bytes is exercised by the fail-closed checker.
    if mutation.startswith("change_operation_") or mutation == "enable_kind29_operation":
        operation = read_json(operation_path)
        rows = operation.get("operations")
        if not isinstance(rows, list) or len(rows) != 30:
            fail("operation registry has no canonical rows to mutate")
        if mutation == "change_operation_name":
            rows[0]["name"] = "AgentKey"
        elif mutation == "change_operation_body_type":
            rows[0]["body_type"] = "BidBodyV1"
        elif mutation == "change_operation_plane":
            rows[27]["plane"] = "agent"
        elif mutation == "change_operation_authority":
            rows[21]["authority"] = "existing-agent"
        elif mutation == "change_operation_nonce":
            rows[5]["nonce_lane"] = "controller"
        elif mutation == "change_operation_status":
            rows[20]["status"] = "candidate-assigned"
        elif mutation == "enable_kind29_operation":
            rows[29]["enabled"] = True
        else:
            fail(f"unknown operation mutation recipe: {mutation}")
    ...
    with tempfile.TemporaryDirectory(prefix="trnm-cev1-registry-mutants-") as temporary:
        ...
        for index, case in enumerate(cases):
            ...
            if not all(
                ...
                for value in (case_id, mutation, target, expected)
            ):
