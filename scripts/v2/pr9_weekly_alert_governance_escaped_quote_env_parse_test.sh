...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/trnm-pr9-escaped-quote-env.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
cat >"$TMP/run/pr9/history/weekly-alert-governance-20260305T000000Z.json" <<'JSON'
{
  ...
  "history_fingerprint_sha256": "prev-escaped-quote-thresholds"
}
JSON
...
print('[OK] pr9 weekly alert governance preserves escaped quotes inside threshold env values')
PY
