...
fn resolve_authority_account(st: &StateStore) -> String {
    st.gov_param_string("resolve_authority")
        // Preserve exact governance payload; apply_resolve enforces canonical
        // non-whitespace account ids to avoid silently normalizing authority.
...
mod tests {
    ...
    #[test]
    fn resolve_authority_rotation_revokes_stale_signer_without_escrow_mutation() {
        let mut st = seeded_state();
        st.set_balance("challenger", 100);
        set_resolve_authority(&mut st, "authority_a");
        set_resolve_authority(&mut st, "authority_b");
        ...
        let before = st.clone();
        let err = apply_resolve(&mut st, r5.clone(), true, "authority_a".into(), "authority_a".into())
            .unwrap_err();
        assert!(matches!(err, PouwError::Unauthorized));
        ...
        let r6 = apply_resolve(&mut st, r5, true, "authority_b".into(), "authority_b".into()).unwrap();
        let task = st.get_task(r6.id).unwrap();
        assert_eq!(task.status, TaskStatus::Slashed);
        assert_eq!(task.challenge_bond_forfeited, Some(false));
        assert_eq!(st.balance_of("challenger"), 101);
