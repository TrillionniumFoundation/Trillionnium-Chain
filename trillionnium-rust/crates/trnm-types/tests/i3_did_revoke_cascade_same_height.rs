...
fn did_revoke_cascade_same_height_fail_closed_for_all_subject_tokens() {
    ...
    reg.verify_capability("org:lane-xi-admin", token_a, CapabilityScope::BridgeSettle, 29)
        .unwrap();
    ...
    assert_eq!(cascade_revokes.iter().map(|ev| ev.at_height).collect::<Vec<_>>(), vec![30, 30]);
}
