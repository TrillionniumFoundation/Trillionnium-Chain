...
# Regression 18: duplicate canonical MV2 proof-family anchor line in snapshot; gate must fail on non-unique anchor line.
python3 - <<'PY' "$SNAPSHOT_SPEC"
from pathlib import Path
import sys
...
if "$GATE" >/dev/null 2>&1; then
  echo "[FAIL] MV2 gate should fail when snapshot has duplicated frozen proof-family anchor line" >&2
  exit 1
fi
