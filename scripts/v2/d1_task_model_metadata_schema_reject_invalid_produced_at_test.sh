...
cat >"$tmp_file" <<'JSON'
{
  ...
  "provenance": {
    ...
    "produced_at": "2026-13-01T01:20:00Z",
...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] expected non-zero exit for produced_at format violation" >&2
  exit 1
fi
...
echo "[PASS] D1 schema gate rejects invalid provenance produced_at date-time"
