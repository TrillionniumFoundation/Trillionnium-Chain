...
cat >"$tmp_file" <<'JSON'
{
  ...
  "provenance": {
    ...
    "produced_at": "2026-03-02T01:45:00Z",
...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] expected non-zero exit for trailing-space model.version" >&2
  exit 1
fi
...
echo "[PASS] D1 schema gate rejects model.version with trailing space"
