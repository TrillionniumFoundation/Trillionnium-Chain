...
function lockConstructorParameters(signature, expected, label) {
  ...
  invariant(
    !/(?:foreign|poco_consensus|request[_a-z]*hash|block[_a-z]*hash|part[_a-z]*set)/i.test(signature),
...
function rejectforeignValues(value, label = "vector") {
  ...
  for (const [key, nested] of Object.entries(value)) {
    ...
    invariant(allowedNullMarker || !/(?:foreign|poco_consensus|request_hash|part_set)/i.test(key), `${label}.${key}: forbidden foreign hash/BlockID value field`);
