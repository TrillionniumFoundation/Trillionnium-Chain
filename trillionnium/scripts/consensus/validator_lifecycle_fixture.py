#!/usr/bin/env python3
"""Build and verify local external BFT engine validator-lifecycle fixtures.

The transition command reads external BFT engine private-validator keys only to create the
...
def validator_set_hash(validators: list[dict[str, Any]]) -> bytes:
    ...
    return framed_hash("trnm.external_bft_engine.validator-set.v1", [encoded])


def read_foreign_private_key(path: Path) -> tuple[str, Ed25519PrivateKey]:
    raw = load_json(path)
    require(isinstance(raw, dict), f"external BFT engine key {path} is not an object")
    public = raw.get("pub_key")
    private = raw.get("priv_key")
    require(isinstance(public, dict), f"external BFT engine key {path} has no public key")
    require(isinstance(private, dict), f"external BFT engine key {path} has no private key")
    require(
        public.get("type") == "external_bft_engine/PubKeyEd25519",
        f"external BFT engine key {path} is not Ed25519",
    )
    require(
        private.get("type") == "external_bft_engine/PrivKeyEd25519",
        f"external BFT engine key {path} is not Ed25519",
    )
    ...
    require(len(expected_public) == 32, f"external BFT engine key {path} public key has wrong length")
    require(
        len(private_bytes) in (32, 64),
        f"external BFT engine key {path} private key has wrong length",
    )
    ...
    require(
        ...
        f"external BFT engine key {path} public/private key mismatch",
    )
...
def public_key_from_foreign_key(path: Path) -> str:
    ...
    require(len(value) == 32, f"external BFT engine key {path} public key has wrong length")
    return value.hex()
...
def command_assert_phase(args: argparse.Namespace) -> None:
    ...
    info = rpc_get(args.rpc_url, "external_application_adapter_info", {})
    response = info.get("response")
    require(isinstance(response, dict), "external application adapter info has no response")
    require(
        int(response.get("last_block_height", -1)) == args.height,
        "external application adapter height does not equal asserted phase height",
    )
    external_application_adapter_hash = normalize_hash(response.get("last_block_app_hash"), "external application adapter app hash")
    ...
    require(local_hash == external_application_adapter_hash, "local and external application adapter app hashes differ")
    ...
    atomic_json(
        args.json_out,
        {
            ...
            "app_hash_hex": external_application_adapter_hash,
        },
    )
