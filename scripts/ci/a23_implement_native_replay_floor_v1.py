#!/usr/bin/env python3
"""Apply the exact A23 native replay-floor source patch.

This script is used only by the branch-scoped one-shot implementation workflow
and is deleted from the resulting candidate tree.
"""
...
native_module.write_text(
    r'''//! Proof-bound signer replay-floor facts for tombstone retention.
//!
...
impl DurableNativeApplicationV0 {
    ...
    pub fn verify_finalized_signer_replay_floor_v1(
        ...
        if !contains_contiguous_signer_prefix_v1(
            &read,
...
node_module.write_text(
    r'''/// A deterministic description of the only native replay-floor policy this
...
durable = Path("trillionnium/crates/trnm-native-execution-v0/src/durable.rs")
replace_once(
    durable,
...
replace_once(
    ...
    """        let receipt_commitments = executed
            ...
            .map(|receipt| Hash32V0::new(*receipt.commitment().as_bytes()))
            .collect::<Vec<_>>();
        let replay_signer_nonces: BTreeSet<(String, u64)> = decode_borsh_v0(
            &p.target_nonce_bytes,
            "read_finalized.target_nonces",
        )?;
        let row = ConfirmedDurableExecutionHistoryRowV0 {""",
)
replace_once(
...
lib = Path("trillionnium/crates/trnm-native-execution-v0/src/lib.rs")
replace_once(
    lib,
    """    NativeApplicationExecutionErrorCodeV0, NativeApplicationExecutionErrorV0,
...
tx = Path("trillionnium/crates/trnm-poco-node/src/tx_admission_wal.rs")
replace_once(
    tx,
...
verify_workflow = Path(".github/workflows/trnm-a23-native-replay-floor-verify.yml")
verify_workflow.write_text(
    r'''name: trnm-a23-native-replay-floor-verify

on:
  push:
    branches:
      - feature/chain-a23-p2-native-replay-floor-v1-20260831
    paths:
      - .github/workflows/trnm-a23-native-replay-floor-verify.yml
...
concurrency:
  group: trnm-a23-native-replay-floor-verify-${{ github.ref }}
  cancel-in-progress: true

jobs:
  verify-native-replay-floor:
    if: >-
      github.repository == 'TrillionniumFoundation/Trillionnium-Chain' &&
      github.ref == 'refs/heads/feature/chain-a23-p2-native-replay-floor-v1-20260831'
    runs-on: ubuntu-24.04
    timeout-minutes: 180
    steps:
      - name: Check out exact A23 candidate
        uses: actions/checkout@11d5960a326750d5838078e36cf38b85af677262
        with:
          ref: feature/chain-a23-p2-native-replay-floor-v1-20260831
          fetch-depth: 0
          persist-credentials: false

      - name: Verify immutable A23 parent and clean authority surface
        run: |
          set -euo pipefail
          test "$(git rev-parse HEAD^)" = "__A23_IMPLEMENTATION_PARENT__"
          git merge-base --is-ancestor 2384e7e4ceb1fe63a0f29e91dab274d74940a8d3 HEAD
          test ! -e .github/workflows/trnm-a23-native-replay-floor-inventory.yml
          test ! -e .github/workflows/trnm-a23-native-replay-floor-implement.yml
          test ! -e scripts/ci/a23_implement_native_replay_floor_v1.py
          ! grep -R --line-number --fixed-strings 'contents: write' \
            .github/workflows/trnm-a23-native-replay-floor-*.yml
      ...
      - name: Format, compile and lint exact candidate
        run: |
          set -euo pipefail
          ...
          cargo check --manifest-path trillionnium/Cargo.toml \
            -p trnm-poco-node --features tx-admission-wal --all-targets --locked
          cargo clippy --manifest-path trillionnium/Cargo.toml \
            -p trnm-native-execution-v0 --all-targets --locked -- -D warnings
          cargo clippy --manifest-path trillionnium/Cargo.toml \
            -p trnm-poco-node --features tx-admission-wal --all-targets --locked -- -D warnings

      - name: Run focused and complete regressions
        run: |
          set -euo pipefail
          cargo test --manifest-path trillionnium/Cargo.toml \
            -p trnm-native-execution-v0 \
            finalized_commit_adapter_runs_signed_tx_to_state_proof_and_reopen_v0 --locked
          cargo test --manifest-path trillionnium/Cargo.toml \
            -p trnm-poco-node --features tx-admission-wal \
            native_replay_floor_purges_only_strictly_finalized_contiguous_prefix_v1 --locked
          cargo test --manifest-path trillionnium/Cargo.toml \
            -p trnm-native-execution-v0 --lib --locked
          cargo test --manifest-path trillionnium/Cargo.toml \
            -p trnm-poco-node --features tx-admission-wal --lib --locked
          cargo test --manifest-path trillionnium/Cargo.toml \
            -p trnm-native-execution-v0 --doc --locked
          cargo test --manifest-path trillionnium/Cargo.toml \
            -p trnm-poco-node --features tx-admission-wal --doc --locked

      - name: Re-run A22 authority policy over the expanded source
        run: |
          set -euo pipefail
      ...
      - name: Confirm exact source tree is clean
        run: |
          set -euo pipefail
          git diff --check
...
package_doc = Path("docs/development/packages/TRNM_A23_P2_NATIVE_REPLAY_FLOOR_V1.md")
package_doc.write_text(
    r'''# TRNM A23 P2 native replay-floor v1
