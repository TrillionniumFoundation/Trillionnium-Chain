...
- CLI audit visibility: `trnm-cli query task <task_id>`, `trnm-cli query events <task_id> --limit N`, and `trnm-cli query request-full <request_id> --limit N` now pretty-print the same nested metering audit block / timeline when the underlying RPC response includes it
