...
- if `ceremony_scope=public-mainnet-input`, require operators to replace `<owner>` / `<chat/email/oncall>` / `<optional-ack-path>` / `<optional-sha256-of-ack>` placeholders before startup instead of relying on a later cleanup pass
