...
production, external application adapter, authenticated cross-epoch, or H3b2b1 closure evidence.
...
### 5.9 B2-H3b2b2 application-authenticated candidate reconstruction (bounded shared and external application adapter/restart evidence landed; remaining closure in progress)
...
- host admission preserves the distinction between terminal whole-block
  invalidity and retryable `Unavailable`; in particular, no external application adapter `REJECT` or
...
- that carrier accepts no caller-supplied height/root/version, second parent or
  ...
  checkpoint, Core callback, vote/finality, or external application adapter authority;
...
- state-sync snapshot creation MUST transactionally scrub callback-outbox rows
  ...
  host/AppCore/external application adapter network wiring, general recovery, or process-wide Core
  uniqueness;
...
- the bounded production validation cursor owns a private fallible
  ...
  host/external application adapter runtime adapter remains unwired, legacy `load_object` remains a
  direct read, and no production external application adapter/outcome path consumes the carrier;
...
- the single consuming comparator MUST rebind retained receipt -> replayed
  ...
  `ExecutionOutcomeV0`, authorized-native-block, checkpoint, Core, or external application adapter
  authority;
- the production exact-transaction cursor can only borrow its store, chain,
  ...
  Core-callback, or external application adapter authority;
...
- every open/decode/runtime/planning failed-close carrier MUST be private,
  ...
  `PayloadValidationResult`, authorized-native-block, checkpoint, Core, or external application adapter
...
- typed historical cutoff/projection reads, the exact estimate-input,
  ...
  invariant host mapping, production host/Core callback wiring, and external application adapter
...
external application adapter/SQLite/cache/restart/V3/V4 plus targeted pruned-cutoff evidence. Its
