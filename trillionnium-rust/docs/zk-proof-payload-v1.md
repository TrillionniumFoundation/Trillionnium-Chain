...
- `backend_id`, when present, must be a non-empty canonical token without surrounding whitespace, must remain a single opaque token without embedded whitespace or control characters (including Unicode whitespace), and must contain at least one visible canonical backend token segment rather than punctuation-only noise; the legacy `noop` selector remains the only supported explicit no-backend alias, and in v0 it must use the exact lowercase canonical token `noop` rather than case-drifted spellings such as `NOOP`.
- `backend_version`, when present, must be a non-empty canonical token without surrounding whitespace, must remain a single opaque token without embedded whitespace or control characters (including Unicode whitespace), must contain at least one visible canonical version token segment rather than punctuation-only noise, and must not appear without `backend_id`.
...
- The canonical JSON object is fail-closed on unknown fields: the top-level payload object, the `public_inputs` object, and the nested `meta` object all reject undeclared keys rather than silently tolerating forward drift.
...
- When a canonical payload explicitly provides `backend_id`, that selector is authoritative for the router path: if the named backend is unknown, disabled, or otherwise unavailable, the result must stay fail-closed as an unavailable route. The router must not silently fall back to the configured default backend, even when fallback-related config knobs are present.
- The same fail-closed vk/system consistency applies to the router-selected backend token, not just the payload field: an explicitly tee-family backend token on the ZK path must be rejected, a family-only `zk` / `zk-*` router token without a canonical zk-system hint must be rejected as malformed rather than treated as an implicit backend alias, the legacy no-backend selector remains the exact lowercase canonical token `noop` (case-drifted router-selected forms such as `NOOP` must also fail closed), router-selected tokens must not rely on silent surrounding-whitespace trimming or embedded whitespace/control normalization (including Unicode whitespace), repeated identical zk-system hints may be deduplicated, and mixed canonical system hints must fail closed rather than being guessed through.
