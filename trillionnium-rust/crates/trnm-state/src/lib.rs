...
mod tests {
    ...
    #[test]
    fn checkpoint_da_light_verifier_summary_fails_closed_on_height_mismatch() {
        let wal = WalMeta {
            height: 7,
            round: 3,
            proposal_hash: "proposal-7".into(),
        ...
        assert_eq!(
            ...
            None,
            "DA/light-verifier summary must fail closed when checkpoint height and WAL height diverge so checkpoint evidence cannot be rebound onto a different canonical slot"
        );
    }
    ...
    fn checkpoint_da_light_verifier_summary_fails_closed_on_uncommitted_wal() {
        ...
        assert_eq!(
            checkpoint_da_light_verifier_summary(&checkpoint, &wal),
            None,
            "DA/light-verifier summary must fail closed on speculative committed=false WAL metadata even when height/state_root/hash otherwise align"
        );
    }
    ...
    #[test]
    fn checkpoint_da_light_verifier_summary_fails_closed_on_noncanonical_prev_hash_surfaces() {
        let canonical_wal = WalMeta {
            height: 7,
            round: 3,
            proposal_hash: "proposal-7".into(),
        ...
        for invalid_prev_hash in [
            ...
            assert_eq!(
                checkpoint_da_light_verifier_summary(&checkpoint, &wal),
                None,
                "DA/light-verifier summary must fail closed when wal prev_hash surface is noncanonical or missing for non-genesis linkage"
            );
        }
    }
