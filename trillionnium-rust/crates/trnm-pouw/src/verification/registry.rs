...
mod tests {
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_style_payload_routes_mixed_case_real_backend_id_with_blank_backend_version_when_feature_enabled() {
        let registry = registry_with_feature_on_fixture_bridge_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
        ...
        let payload = br#"ZK:{"task_id":42,"worker":"worker-zk","proof_type":"zk","result_hash":"1111111111111111111111111111111111111111111111111111111111111111","zk_system":"groth16","backend_id":" \n\tREAL-ZK-BACKEND\t ","backend_version":"   \n\t  ","vk_ref":"vk://trnm/dev/mock-groth16/valid","proof_encoding":"hex","proof":"01020304","public_inputs":{"order":["task_id","proof_type","worker","result_hash"],"values":["42","zk","worker-zk","1111111111111111111111111111111111111111111111111111111111111111"]},"meta":{"schema_version":"trnm.zk.payload.v0","circuit_id":"fixture-bridge-groth16-inline-mixed-case-blank-version-feature-on"}}"#;
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_file_routes_canonical_whitespace_padded_real_backend_id_with_null_backend_version_when_feature_enabled() {
        let registry = registry_with_feature_on_fixture_bridge_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
        ...
        let payload = format!(
            "ZK:{}",
            include_str!("../../fixtures/zk/real_backend_bridge_groth16_feature_on_canonical_whitespace_backend_id_null_backend_version.json").trim()
        );
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_file_routes_canonical_whitespace_padded_real_backend_id_with_null_backend_version_for_plonk_when_feature_enabled() {
        let registry = registry_with_feature_on_fixture_bridge_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
        ...
        let payload = format!(
            "ZK:{}",
            include_str!("../../fixtures/zk/real_backend_bridge_plonk_feature_on_canonical_whitespace_backend_id_null_backend_version.json").trim()
        );
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_file_routes_canonical_whitespace_padded_real_backend_id_with_padded_nonempty_backend_version_when_feature_enabled() {
        let registry = registry_with_feature_on_fixture_bridge_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
        ...
        let payload = format!(
            "ZK:{}",
            include_str!("../../fixtures/zk/real_backend_bridge_groth16_feature_on_canonical_whitespace_backend_id_padded_version.json").trim()
        );
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_file_routes_canonical_whitespace_padded_real_backend_id_with_padded_nonempty_backend_version_for_plonk_when_feature_enabled() {
        let registry = registry_with_feature_on_fixture_bridge_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
        ...
        let payload = format!(
            "ZK:{}",
            include_str!("../../fixtures/zk/real_backend_bridge_plonk_feature_on_canonical_whitespace_backend_id_padded_version.json").trim()
        );
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_style_payload_routes_mixed_case_real_backend_id_with_null_backend_version_for_plonk_when_feature_enabled() {
        let registry = registry_with_feature_on_fixture_bridge_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
        ...
        let payload = br#"ZK:{"task_id":42,"worker":"worker-zk","proof_type":"zk","result_hash":"1111111111111111111111111111111111111111111111111111111111111111","zk_system":"plonk","backend_id":"\n\tREAL-ZK-BACKEND  ","backend_version":null,"vk_ref":"vk://trnm/dev/mock-plonk/valid","proof_encoding":"hex","proof":"01020304","public_inputs":{"order":["task_id","proof_type","worker","result_hash"],"values":["42","zk","worker-zk","1111111111111111111111111111111111111111111111111111111111111111"]},"meta":{"schema_version":"trnm.zk.payload.v0","circuit_id":"fixture-bridge-plonk-inline-mixed-case-null-version-feature-on"}}"#;
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_file_accepts_null_backend_version_for_canonical_real_backend_id_when_feature_enabled() {
        let registry = registry_with_mock_zk_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
        ...
        let payload = format!(
            "ZK:{}",
            include_str!("../../fixtures/zk/real_backend_bridge_groth16_feature_on_null_backend_version.json").trim()
        );
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_file_accepts_null_backend_version_for_canonical_real_backend_id_for_plonk_when_feature_enabled() {
        let registry = registry_with_mock_zk_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
        ...
        let payload = format!(
            "ZK:{}",
            include_str!("../../fixtures/zk/real_backend_bridge_plonk_feature_on_null_backend_version.json").trim()
        );
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_style_payload_accepts_canonical_whitespace_backend_id_with_padded_version_without_selecting_real_backend() {
        let registry = registry_with_mock_zk_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
        ...
        let payload = br#"ZK:{"task_id":42,"worker":"worker-zk","proof_type":"zk","result_hash":"1111111111111111111111111111111111111111111111111111111111111111","zk_system":"groth16","backend_id":"  \n\treal-zk-backend\t ","backend_version":" \n\t2026-03-12-fixture-bridge\t ","vk_ref":"vk://trnm/dev/mock-groth16/valid","proof_encoding":"hex","proof":"01020304","public_inputs":{"order":["task_id","proof_type","worker","result_hash"],"values":["42","zk","worker-zk","1111111111111111111111111111111111111111111111111111111111111111"]},"meta":{"schema_version":"trnm.zk.payload.v0","circuit_id":"fixture-bridge-groth16-inline-canonical-whitespace-padded-version-without-selecting-real-backend"}}"#;
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_style_payload_accepts_canonical_whitespace_backend_id_with_blank_version_without_selecting_real_backend() {
        let registry = registry_with_mock_zk_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
        ...
        let payload = br#"ZK:{"task_id":42,"worker":"worker-zk","proof_type":"zk","result_hash":"1111111111111111111111111111111111111111111111111111111111111111","zk_system":"groth16","backend_id":"  \n\treal-zk-backend\t ","backend_version":"   \n\t  ","vk_ref":"vk://trnm/dev/mock-groth16/valid","proof_encoding":"hex","proof":"01020304","public_inputs":{"order":["task_id","proof_type","worker","result_hash"],"values":["42","zk","worker-zk","1111111111111111111111111111111111111111111111111111111111111111"]},"meta":{"schema_version":"trnm.zk.payload.v0","circuit_id":"fixture-bridge-groth16-inline-canonical-whitespace-blank-version-without-selecting-real-backend"}}"#;
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_style_payload_accepts_canonical_whitespace_backend_id_with_blank_version_for_plonk_without_selecting_real_backend() {
        let registry = registry_with_mock_zk_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
        ...
        let payload = br#"ZK:{"task_id":42,"worker":"worker-zk","proof_type":"zk","result_hash":"1111111111111111111111111111111111111111111111111111111111111111","zk_system":"plonk","backend_id":"  \n\treal-zk-backend\t ","backend_version":"   \n\t  ","vk_ref":"vk://trnm/dev/mock-plonk/valid","proof_encoding":"hex","proof":"01020304","public_inputs":{"order":["task_id","proof_type","worker","result_hash"],"values":["42","zk","worker-zk","1111111111111111111111111111111111111111111111111111111111111111"]},"meta":{"schema_version":"trnm.zk.payload.v0","circuit_id":"fixture-bridge-plonk-inline-canonical-whitespace-blank-version-without-selecting-real-backend"}}"#;
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_style_payload_accepts_canonical_whitespace_backend_id_with_empty_string_version_without_selecting_real_backend() {
        let registry = registry_with_mock_zk_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
        ...
        let payload = br#"ZK:{"task_id":42,"worker":"worker-zk","proof_type":"zk","result_hash":"1111111111111111111111111111111111111111111111111111111111111111","zk_system":"groth16","backend_id":"  \n\treal-zk-backend\t ","backend_version":"","vk_ref":"vk://trnm/dev/mock-groth16/valid","proof_encoding":"hex","proof":"01020304","public_inputs":{"order":["task_id","proof_type","worker","result_hash"],"values":["42","zk","worker-zk","1111111111111111111111111111111111111111111111111111111111111111"]},"meta":{"schema_version":"trnm.zk.payload.v0","circuit_id":"fixture-bridge-groth16-inline-canonical-whitespace-empty-string-version-without-selecting-real-backend"}}"#;
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_style_payload_accepts_canonical_whitespace_backend_id_with_whitespace_only_version_without_selecting_real_backend() {
        let registry = registry_with_mock_zk_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
        ...
        let payload = br#"ZK:{"task_id":42,"worker":"worker-zk","proof_type":"zk","result_hash":"1111111111111111111111111111111111111111111111111111111111111111","zk_system":"groth16","backend_id":"  \n\treal-zk-backend\t ","backend_version":" \n\t ","vk_ref":"vk://trnm/dev/mock-groth16/valid","proof_encoding":"hex","proof":"01020304","public_inputs":{"order":["task_id","proof_type","worker","result_hash"],"values":["42","zk","worker-zk","1111111111111111111111111111111111111111111111111111111111111111"]},"meta":{"schema_version":"trnm.zk.payload.v0","circuit_id":"fixture-bridge-groth16-inline-canonical-whitespace-whitespace-only-version-without-selecting-real-backend"}}"#;
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_style_payload_accepts_canonical_whitespace_backend_id_with_whitespace_only_version_for_plonk_without_selecting_real_backend() {
        let registry = registry_with_mock_zk_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
        ...
        let payload = br#"ZK:{"task_id":42,"worker":"worker-zk","proof_type":"zk","result_hash":"1111111111111111111111111111111111111111111111111111111111111111","zk_system":"plonk","backend_id":"  \n\treal-zk-backend\t ","backend_version":" \n\t ","vk_ref":"vk://trnm/dev/mock-plonk/valid","proof_encoding":"hex","proof":"01020304","public_inputs":{"order":["task_id","proof_type","worker","result_hash"],"values":["42","zk","worker-zk","1111111111111111111111111111111111111111111111111111111111111111"]},"meta":{"schema_version":"trnm.zk.payload.v0","circuit_id":"fixture-bridge-plonk-inline-canonical-whitespace-whitespace-only-version-without-selecting-real-backend"}}"#;
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_file_accepts_canonical_whitespace_backend_id_with_padded_version_without_selecting_real_backend() {
        let registry = registry_with_mock_zk_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_file_accepts_canonical_whitespace_backend_id_with_padded_version_for_plonk_without_selecting_real_backend() {
        let registry = registry_with_mock_zk_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_file_accepts_canonical_whitespace_backend_id_with_empty_string_version_for_plonk_without_selecting_real_backend() {
        let registry = registry_with_mock_zk_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_file_accepts_canonical_whitespace_backend_id_with_null_backend_version_without_selecting_real_backend() {
        let registry = registry_with_mock_zk_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_file_accepts_canonical_whitespace_backend_id_with_null_backend_version_for_plonk_without_selecting_real_backend() {
        let registry = registry_with_mock_zk_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_style_payload_accepts_canonical_whitespace_backend_id_with_empty_string_version_for_plonk_without_selecting_real_backend() {
        let registry = registry_with_mock_zk_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
        ...
        let payload = br#"ZK:{"task_id":42,"worker":"worker-zk","proof_type":"zk","result_hash":"1111111111111111111111111111111111111111111111111111111111111111","zk_system":"plonk","backend_id":"  \n\treal-zk-backend\t ","backend_version":"","vk_ref":"vk://trnm/dev/mock-plonk/valid","proof_encoding":"hex","proof":"01020304","public_inputs":{"order":["task_id","proof_type","worker","result_hash"],"values":["42","zk","worker-zk","1111111111111111111111111111111111111111111111111111111111111111"]},"meta":{"schema_version":"trnm.zk.payload.v0","circuit_id":"fixture-bridge-plonk-inline-canonical-whitespace-empty-string-version-without-selecting-real-backend"}}"#;
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_style_payload_accepts_canonical_whitespace_backend_id_with_null_backend_version_without_selecting_real_backend() {
        let registry = registry_with_mock_zk_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
        ...
        let payload = br#"ZK:{"task_id":42,"worker":"worker-zk","proof_type":"zk","result_hash":"1111111111111111111111111111111111111111111111111111111111111111","zk_system":"groth16","backend_id":"  \n\treal-zk-backend\t ","backend_version":null,"vk_ref":"vk://trnm/dev/mock-groth16/valid","proof_encoding":"hex","proof":"01020304","public_inputs":{"order":["task_id","proof_type","worker","result_hash"],"values":["42","zk","worker-zk","1111111111111111111111111111111111111111111111111111111111111111"]},"meta":{"schema_version":"trnm.zk.payload.v0","circuit_id":"fixture-bridge-groth16-inline-canonical-whitespace-null-backend-version-without-selecting-real-backend"}}"#;
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_fixture_style_payload_accepts_canonical_whitespace_backend_id_with_null_backend_version_for_plonk_without_selecting_real_backend() {
        let registry = registry_with_mock_zk_backend();
        let mut task = task_with_proof_type(ProofType::Zk);
        task.status = TaskStatus::Committed;
        task.worker = Some("worker-zk".into());
        ...
        let payload = br#"ZK:{"task_id":42,"worker":"worker-zk","proof_type":"zk","result_hash":"1111111111111111111111111111111111111111111111111111111111111111","zk_system":"plonk","backend_id":"  \n\treal-zk-backend\t ","backend_version":null,"vk_ref":"vk://trnm/dev/mock-plonk/valid","proof_encoding":"hex","proof":"01020304","public_inputs":{"order":["task_id","proof_type","worker","result_hash"],"values":["42","zk","worker-zk","1111111111111111111111111111111111111111111111111111111111111111"]},"meta":{"schema_version":"trnm.zk.payload.v0","circuit_id":"fixture-bridge-plonk-inline-canonical-whitespace-null-backend-version-without-selecting-real-backend"}}"#;
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_all_feature_on_bridge_fixtures_route_cleanly_when_selected() {
        use std::fs;
        ...
        assert!(
            fixture_paths.len() >= 8,
            "expected multiple feature-on bridge fixtures, found {}",
            fixture_paths.len()
        );
        ...
        for fixture_path in fixture_paths {
            ...
            assert_eq!(
                registry.verify(&task, payload.as_bytes()),
                VerificationResult::Valid,
                "fixture should verify cleanly: {fixture_name}"
            );
        }
    }
    ...
    fn registry_zk_vector_all_invalid_feature_on_bridge_fixtures_fail_closed_when_selected() {
        ...
        for fixture_path in fixture_paths {
            ...
            assert!(
                ...
                "fixture should fail closed for missing backend_id: {fixture_name}"
            );
        }
    }
    ...
    #[cfg(feature = "real-zk-backend")]
    #[test]
    fn registry_zk_vector_invalid_feature_on_bridge_fixture_matrix_keeps_groth16_plonk_suffix_pairs_in_sync(
    ) {
        use std::collections::BTreeMap;
        use std::fs;
        use std::path::PathBuf;
        ...
        assert!(
            suffix_families.len() >= 3,
            "expected invalid backend-id feature-on suffix coverage, found {} classes",
            suffix_families.len()
        );
        ...
        for (suffix, mut families) in suffix_families {
            ...
            assert_eq!(
                families,
                vec!["groth16".to_string(), "plonk".to_string()],
                "invalid feature-on bridge fixtures drifted for suffix '{suffix}'"
            );
        }
    }
