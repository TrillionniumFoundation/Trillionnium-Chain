...
- Reuse rule: when citing this board in a release / RC / handoff context, also record the then-current `git rev-parse origin/main` output instead of assuming the snapshot above is still current.
