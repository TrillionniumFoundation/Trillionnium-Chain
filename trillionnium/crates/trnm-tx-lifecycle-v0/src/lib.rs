...
impl TxIntentV0 {
    pub fn validate(&self) -> Result<(), TxLifecycleErrorV0> {
        ...
        if self.authorization.is_empty()
            || self.authorization.len() > MAX_AUTHORIZATION_BYTES_V0
        {
    ...
    V: AuthorizationVerifierV0,
{
    ...
    pub fn admit(
        ...
        intent.validate().map_err(TxLifecycleHostErrorV0::Lifecycle)?;
        ...
        if let Some(previous_id) = self.active_nonce.get(&nonce_key).copied() {
            let previous = self.records.get(&previous_id).ok_or(
                TxLifecycleHostErrorV0::Lifecycle(TxLifecycleErrorV0::StateCorruption),
            )?;
            if !matches!(previous.phase, TxPhaseV0::Admitted | TxPhaseV0::WalPersisted)
            ...
            let previous = self.records.get_mut(&previous_id).ok_or(
                TxLifecycleHostErrorV0::Lifecycle(TxLifecycleErrorV0::StateCorruption),
            )?;
            previous
...
impl fmt::Display for TxLifecycleErrorV0 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(match self {
            ...
            Self::ReplacementRejected => "transaction replacement is not strictly fee-increasing and pre-order",
            ...
            Self::ExecutionReceiptMismatch => "execution receipt is not bound to the ordered transaction",
...
impl<A> Error for TxLifecycleHostErrorV0<A>
where
    A: Error + 'static,
{
}
...
mod tests {
    ...
    fn replacement_requires_strictly_higher_fee_and_preorder_phase() {
        ...
        assert_eq!(lifecycle.record(first).unwrap().phase, TxPhaseV0::Tombstoned);
        assert_eq!(
