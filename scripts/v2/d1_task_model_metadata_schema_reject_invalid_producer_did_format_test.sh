...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] expected non-zero exit for producer_did format violation" >&2
  exit 1
fi
...
echo "[PASS] D1 schema gate rejects invalid provenance producer_did format"
