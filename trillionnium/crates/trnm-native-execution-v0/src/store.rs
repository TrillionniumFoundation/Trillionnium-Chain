...
impl InMemoryNativeExecutionStoreV0 {
    ...
    /// Read one raw key under an explicitly retained root. Snapshot recovery
    /// still validates all live values; a consumer needing one lifecycle record
    /// need not build and re-prove the entire live map a second time.
