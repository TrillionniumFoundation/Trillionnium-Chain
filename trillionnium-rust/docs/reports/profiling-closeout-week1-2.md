...
- 报告会同时给出：
  ...
  - `Benchmark Pool Action Summary`：把三个 bench 候选池再聚合成一段总览，输出各类 `status` / `action` 计数、`benchmark_pool_attention`，以及 `benchmark_pool_followup_command_chain`，方便 autopilot 直接决定下一步是补产物、刷新产物，还是保留 latest 并考虑归档旧 backlog
