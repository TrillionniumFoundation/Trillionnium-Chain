...
Status: **BLOCKED_UPSTREAM / candidate-only**. This is a subordinate package
contract and gap ledger for A16. It is not a protocol or normative freeze, a
Gate exit, a release decision, a private-alpha authorization, or a production
activation record.
...
The control/documentation branch was live-revalidated at
`origin/docs/agent-fleet-plan-v1-20260829@8bfd73f0cf1b785a29ae212f13212e51fe34231`
(`cfedd363147934f50d1352dae31b7d87d79aa8d9`) during this run. Its assessed
Plan commit/tree and all five locked digests above remain unchanged, so this is
recorded as control-plane drift rather than a candidate-source `BASE_DRIFT`.
...
This package currently claims only that the existing candidate surfaces and
missing interfaces have been inventoried. Existing local observations include a
five-store fresh-readback join, a bounded Rust Order verifier, a candidate
composite-root/pre-vote seam, manifest-bound process tests, and a local Order
tag-50 membership writer. They are useful candidate inputs, not authority.
...
environment used for this documentation run has no `cargo` executable, so the
Cargo build/test/clippy commands are recorded as **not run** rather than as a
pass or failure of the source.
...
`WholeNodeAnchorV1` (candidate code names:
`PocoNodeG2fAnchorRecordV1` and
`PocoNodeG2fExternalMonotonicAnchorV1`) binds generation, finalized height,
...
Documentation-run result on 2026-08-29:
...
control ref: live 8bfd73f0cf1b785a29ae212f13212e51fe34231 /
            cfedd363147934f50d1352dae31b7d87d79aa8d9 (control-plane drift only)
...
| `G2F-ANCHOR-001` | P0 | working | A16-owned (coordinate A05/A17) | candidate contract/unit mutants; no backend | finish external monotonic anchor and rollback matrix |
| `G2F-NAMESPACE-001` | P0 | working | A16-owned | candidate descriptor/openat contract; process integration absent | finish race mutants and independent replay |
...
No row is `closed-candidate` in this documentation-only run. A missing or
changed upstream interface creates a typed ICR and keeps the integration rows
`blocked-upstream`; A16 must not edit the other owner's source semantics.
...
Until then the honest package terminal state is `BLOCKED_UPSTREAM` (with local
`open` rows retained), not Gate acceptance.
...
1. send `G2F-ICR-A11-001` through `G2F-ICR-A15-001` to the owning agents;
2. while waiting, close the highest-severity unblocked local row
   `G2F-NAMESPACE-001` with a candidate-only descriptor/openat fixture and
   retained same-UID/copy/ancestor mutants;
3. revalidate the exact candidate commit/tree and all plan/machine digests;
