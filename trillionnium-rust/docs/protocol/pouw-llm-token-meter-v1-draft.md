...
- CLI audit visibility: `trnm-cli query task <task_id>` and `trnm-cli query events <task_id> --limit N` now pretty-print the same nested metering audit block when the underlying RPC response includes it
