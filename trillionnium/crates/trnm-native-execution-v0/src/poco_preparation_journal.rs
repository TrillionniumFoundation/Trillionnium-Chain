...
//! it is not part of `STORE_SCHEMA_SQL`, canonical JMT state, transport snapshots, or
...
use crate::{
    poco_checkpoint::native_checkpoint_execution_authorization_id_v0,
...
// Local operational ceilings, not consensus limits or evidence of invalidity.
// No eviction or destructive compaction is authorized by reaching a ceiling.
const MAX_JOURNAL_TRANSITIONS: u64 = 64;
const MAX_JOURNAL_PREPARATIONS: u64 = 1024;
const MAX_JOURNAL_SCAN_BYTES: u64 = 256 * 1024 * 1024;
const MAX_JOURNAL_DATABASE_BYTES: u64 = 512 * 1024 * 1024;

fn resource_unavailable(reason: &'static str) -> anyhow::Error {
    io::Error::new(io::ErrorKind::StorageFull, reason).into()
}

fn is_resource_unavailable(error: &anyhow::Error) -> bool {
    error
...
/// external engine hash and no signature or vote authorization.
#[derive(Clone, Debug, Eq, PartialEq)]
...
struct PocoPreparationJournalSharedStateV0 {
    ...
    #[cfg(test)]
    after_commit: Mutex<Option<PreparationAfterCommitFaultV0>>,
}

#[cfg(test)]
#[derive(Debug)]
enum PreparationAfterCommitFaultV0 {
    Replace(PathBuf),
    DeleteReservation,
    DeleteBound,
}
...
impl PocoPreparationJournalV0 {
    pub(crate) fn open(database_path: impl AsRef<Path>) -> Result<Self> {
        ...
        if let Some(shared) = matching_state {
            ...
            if let Err(error) = validate_database(&connection) {
                if !is_resource_unavailable(&error) {
    ...
    pub(crate) fn reserve(
        ...
        validate_resource_budget(&transaction)?;
        ...
        if let Some((stored_record, stored_checksum)) = stored_binding {
            ...
            ensure_resource_growth(&transaction, 1, 0, binding_record.len() as u64)?;
        ...
        if let Some((stored_record, stored_checksum)) = stored_preparation {
            ...
            ensure_resource_growth(&transaction, 0, 1, preparation_record.len() as u64)?;
            let inserted = transaction.execute(
        ...
        let reservation = PocoPreparationReservationV0 {
    ...
    pub(crate) fn bind(
        ...
        match (phase, stored_bound, stored_bound_checksum) {
            (0, None, None) => {
                ...
                ensure_resource_growth(&transaction, 0, 0, bound_record.len() as u64)?;
        ...
        self.commit_and_confirm(
            ...
            reservation,
            &preparation_bytes,
            &preparation.binding.canonical_bytes()?,
            Some(&bound_record),
        )
    }

    fn commit_and_confirm(
        &self,
        transaction: Transaction<'_>,
        reservation: &PocoPreparationReservationV0,
        expected_preparation: &[u8],
        expected_binding: &[u8],
        expected_bound: Option<&[u8]>,
    ) -> Result<()> {
        let result = (|| {
            transaction.commit()?;
            #[cfg(test)]
            self.inject_after_commit_fault()?;
            // A SELECT in the writing transaction is not a durable readback.
            // Reopen the named database after commit, verify its identity and
            // audit the exact row under a new SQLite transaction before a
            // reservation or bound capability can leave this owner.
            let mut connection = self.connect()?;
            let read = connection.transaction_with_behavior(TransactionBehavior::Immediate)?;
            validate_transaction_environment(&read, &self.shared)?;
            validate_resource_budget(&read)?;
            ensure_not_halted_connection(&read, &self.shared.sticky_halt)?;
            let binding: (Vec<u8>, Vec<u8>) = read.query_row(
                "SELECT binding_record, binding_checksum FROM transition_bindings WHERE transition_key=?1",
                params![reservation.slot.transition_key.as_slice()],
                |row| Ok((row.get(0)?, row.get(1)?)),
            )?;
            ensure!(
                binding.0 == expected_binding
                    && binding.1.as_slice()
                        == hash_domain(BINDING_CHECKSUM_DOMAIN_V0, &[expected_binding]),
                "fresh preparation transition readback mismatch"
            );
            let height = reservation.slot.height.get().to_be_bytes();
            let view = reservation.slot.view.get().to_be_bytes();
            let stored: StoredPreparationRowV0 = read.query_row(
                "SELECT preparation_record, preparation_checksum, bound_record, bound_checksum, phase
                 FROM preparations WHERE transition_key=?1 AND block_kind=?2 AND height_be=?3 AND view_be=?4",
                params![reservation.slot.transition_key.as_slice(), i64::from(reservation.slot.block_kind),
                    height.as_slice(), view.as_slice()],
                |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?, row.get(4)?)),
            )?;
            ensure!(
                stored.0 == expected_preparation
                    && stored.1.as_slice() == reservation.preparation_checksum
                    && hash_domain(PREPARATION_CHECKSUM_DOMAIN_V0, &[&stored.0])
                        == reservation.preparation_checksum,
                "fresh preparation record readback mismatch"
            );
            let preparation = PocoCheckpointPreparationReplayRecordV0::decode_exact(&stored.0)?;
            ensure!(
                preparation.preparation_id() == reservation.preparation_id,
                "fresh preparation authorization ID mismatch"
            );
            match (stored.4, stored.2.as_deref(), stored.3.as_deref()) {
                (0, None, None) if expected_bound.is_none() => {}
                (1, Some(bound), Some(checksum)) => {
                    ensure!(
                        checksum == hash_domain(BOUND_CHECKSUM_DOMAIN_V0, &[bound]),
                        "fresh bound checksum mismatch"
                    );
                    if let Some(expected) = expected_bound {
                        ensure!(bound == expected, "fresh exact bound record mismatch");
                    }
                    PocoCheckpointBoundReplayRecordV0::decode_exact(bound, &preparation)?;
                }
                _ => bail!("fresh preparation phase or bound record mismatch"),
            }
            ensure!(
                journal_file_identity(&self.database_path)? == self.shared.file_identity,
                "preparation journal namespace changed after durable readback"
            );
            read.commit()?;
            validate_path_resource_budget(&self.database_path)?;
            Ok(())
        })();
        if result.is_err() {
            // This includes successful commit followed by lost/replaced
            // readback. The caller receives no durable capability and every
            // process-local handle remains fenced until process recovery.
    ...
    #[cfg(test)]
    fn inject_after_commit_fault(&self) -> Result<()> {
        let fault = self.shared.after_commit.lock().unwrap().take();
        match fault {
            Some(PreparationAfterCommitFaultV0::Replace(replacement)) => {
                fs::rename(
                    &self.database_path,
                    self.database_path.with_extension("displaced"),
                )?;
                fs::rename(replacement, &self.database_path)?;
            }
            Some(PreparationAfterCommitFaultV0::DeleteReservation) => {
                Connection::open(&self.database_path)?.execute("DELETE FROM preparations", [])?;
            }
            Some(PreparationAfterCommitFaultV0::DeleteBound) => {
                Connection::open(&self.database_path)?.execute(
                    "UPDATE preparations SET bound_record=NULL, bound_checksum=NULL, phase=0",
                    [],
                )?;
            }
            None => {}
...
fn validate_path_resource_budget(path: &Path) -> Result<()> {
    let mut total = 0_u64;
        ...
        match fs::metadata(PathBuf::from(name)) {
            Ok(metadata) => {
                total = total.checked_add(metadata.len()).ok_or_else(|| {
                    resource_unavailable("PoCO preparation journal physical byte count overflow")
                })?;
            }
    ...
    if total > MAX_JOURNAL_DATABASE_BYTES {
        return Err(resource_unavailable(
            "PoCO preparation journal database/sidecars exceed local byte budget",
        ));
    }
    Ok(())
}
...
struct PreparationResourceUsageV0 {
    transitions: u64,
    preparations: u64,
    scan_bytes: u64,
}

fn validate_resource_budget(connection: &Connection) -> Result<PreparationResourceUsageV0> {
    ...
    if page_count
        ...
        .is_none_or(|bytes| bytes > MAX_JOURNAL_DATABASE_BYTES)
    {
        return Err(resource_unavailable(
            "PoCO preparation journal database exceeds local page budget",
        ));
    }
    // Stop counting at limit+1 rather than scanning an attacker-sized table.
    // Only after counts pass do bounded SQL length aggregates inspect blobs;
    // no BLOB is copied into a Vec before this gate.
    let transitions: u64 = connection.query_row(
        "SELECT COUNT(*) FROM (SELECT 1 FROM transition_bindings LIMIT ?1)",
        params![MAX_JOURNAL_TRANSITIONS + 1],
        |row| row.get(0),
    )?;
    let preparations: u64 = connection.query_row(
        "SELECT COUNT(*) FROM (SELECT 1 FROM preparations LIMIT ?1)",
        params![MAX_JOURNAL_PREPARATIONS + 1],
        |row| row.get(0),
    )?;
    if transitions > MAX_JOURNAL_TRANSITIONS || preparations > MAX_JOURNAL_PREPARATIONS {
        return Err(resource_unavailable(
            "PoCO preparation journal exceeds local row budget",
        ));
    }
    let scan_bytes: u64 = connection.query_row(
        "SELECT
          COALESCE((SELECT SUM(length(binding_record)+64) FROM transition_bindings),0)
          + COALESCE((SELECT SUM(length(preparation_record)+COALESCE(length(bound_record),0)+136) FROM preparations),0)",
        [], |row| row.get(0))?;
    if scan_bytes > MAX_JOURNAL_SCAN_BYTES {
        return Err(resource_unavailable(
            "PoCO preparation journal exceeds local audit byte budget",
        ));
    }
    Ok(PreparationResourceUsageV0 {
        transitions,
        preparations,
        scan_bytes,
    })
...
fn ensure_resource_growth(
    connection: &Connection,
    transitions: u64,
    preparations: u64,
    bytes: u64,
) -> Result<()> {
    let usage = validate_resource_budget(connection)?;
    let added_bytes = bytes
        .checked_add(transitions.saturating_mul(64))
        .and_then(|bytes| bytes.checked_add(preparations.saturating_mul(136)));
    if usage
        .transitions
        .checked_add(transitions)
        .is_none_or(|count| count > MAX_JOURNAL_TRANSITIONS)
        || usage
            .preparations
            .checked_add(preparations)
            .is_none_or(|count| count > MAX_JOURNAL_PREPARATIONS)
        || added_bytes
            .and_then(|bytes| usage.scan_bytes.checked_add(bytes))
            .is_none_or(|bytes| bytes > MAX_JOURNAL_SCAN_BYTES)
    {
        return Err(resource_unavailable(
            "PoCO preparation journal has no local capacity for this mutation",
        ));
    }
    Ok(())
}
...
fn configure_connection(connection: &Connection, initialize: bool) -> Result<()> {
    ...
    let page_size: u64 = connection.query_row("PRAGMA page_size", [], |row| row.get(0))?;
    ensure!(page_size > 0, "invalid preparation journal page size");
    let maximum_pages = MAX_JOURNAL_DATABASE_BYTES / page_size;
    let installed_pages: u64 = connection.query_row(
        &format!("PRAGMA max_page_count={maximum_pages}"),
        [],
        |row| row.get(0),
    )?;
    if installed_pages > maximum_pages {
        return Err(resource_unavailable(
            "PoCO preparation journal cannot install local page ceiling",
        ));
    }
...
mod tests {
    ...
    #[test]
    fn post_commit_reservation_and_bound_readback_cannot_publish_stale_capabilities() {
        for (binding, replace) in [(false, false), (false, true), (true, false), (true, true)] {
            let directory = TestDirectory::new();
            let path = directory.join("preparation.sqlite3");
            let replacement = directory.join("replacement.sqlite3");
            drop(PocoPreparationJournalV0::open(&replacement).unwrap());
            let journal = PocoPreparationJournalV0::open(&path).unwrap();
            let independent = PocoPreparationJournalV0::open(&path).unwrap();
            let record = fixture(31);
            let reservation = binding.then(|| journal.reserve(&record).unwrap());
            let fault = if replace {
                PreparationAfterCommitFaultV0::Replace(replacement)
            } else if binding {
                PreparationAfterCommitFaultV0::DeleteBound
            } else {
                PreparationAfterCommitFaultV0::DeleteReservation
            };
            *journal.shared.after_commit.lock().unwrap() = Some(fault);
            let result = match reservation {
                Some(reservation) => journal.bind(&reservation, &bound_fixture(&record)),
                None => journal.reserve(&record).map(|_| ()),
            };
            assert!(result.is_err(), "binding={binding}, replace={replace}");
    ...
    #[test]
    fn preparation_row_cap_preserves_exact_retry_without_eviction_or_validity_failure() {
        let directory = TestDirectory::new();
        let path = directory.join("preparation.sqlite3");
        let journal = PocoPreparationJournalV0::open(&path).unwrap();
        ...
        // Populate legitimate higher-view records through the same canonical
        // fixture constructor, bypassing only repeated O(n) audit work in test
        // setup. The actual rejecting reserve/retry paths use real journal I/O.
        for offset in 1..MAX_JOURNAL_PREPARATIONS {
            let next = fixture(31 + offset);
            let bytes = next.canonical_bytes().unwrap();
            let checksum = hash_domain(PREPARATION_CHECKSUM_DOMAIN_V0, &[&bytes]);
            let slot = next.slot();
            transaction
                .execute(
                    "INSERT INTO preparations(transition_key,block_kind,height_be,view_be,
                    preparation_record,preparation_checksum,bound_record,bound_checksum,phase)
                 VALUES (?1,?2,?3,?4,?5,?6,NULL,NULL,0)",
                    params![
                        slot.transition_key.as_slice(),
                        i64::from(slot.block_kind),
                        slot.height.get().to_be_bytes().as_slice(),
                        slot.view.get().to_be_bytes().as_slice(),
                        bytes,
                        checksum.as_slice()
                    ],
                )
        ...
        let next = fixture(31 + MAX_JOURNAL_PREPARATIONS);
        let error = journal.reserve(&next).unwrap_err();
        assert!(is_resource_unavailable(&error), "{error:#}");
        assert!(!journal.is_halted().unwrap());
        assert_eq!(
            validate_resource_budget(&connection).unwrap().preparations,
            MAX_JOURNAL_PREPARATIONS
        );
        let retry = journal.reserve(&first).unwrap();
        assert_eq!(retry.preparation_checksum, reservation.preparation_checksum);
        journal.bind(&retry, &bound_fixture(&first)).unwrap();
        assert_eq!(
            validate_resource_budget(&connection).unwrap().preparations,
            MAX_JOURNAL_PREPARATIONS
        );
    ...
    #[test]
    fn oversized_database_is_local_unavailability_before_sqlite_or_blob_decode() {
        let directory = TestDirectory::new();
        let path = directory.join("preparation.sqlite3");
        let journal = PocoPreparationJournalV0::open(&path).unwrap();
        let record = fixture(31);
        ...
        // Sparse extension does not allocate a half-gigabyte test artifact.
        file.set_len(MAX_JOURNAL_DATABASE_BYTES + 1).unwrap();
        let error = journal.reserve(&record).unwrap_err();
        assert!(is_resource_unavailable(&error), "{error:#}");
        assert!(!journal.shared.sticky_halt.load(Ordering::Acquire));
    ...
    #[test]
    fn projected_scan_and_transition_growth_fail_before_writes() {
        let directory = TestDirectory::new();
        let path = directory.join("preparation.sqlite3");
        let journal = PocoPreparationJournalV0::open(&path).unwrap();
        let record = fixture(31);
        journal.reserve(&record).unwrap();
        let connection = Connection::open(&path).unwrap();
        let before = validate_resource_budget(&connection).unwrap();
        for (transitions, bytes) in [
            (MAX_JOURNAL_TRANSITIONS, 0),
            (0, MAX_JOURNAL_SCAN_BYTES),
            (0, u64::MAX),
        ] {
            let error = ensure_resource_growth(&connection, transitions, 0, bytes).unwrap_err();
            assert!(is_resource_unavailable(&error));
        }
        let after = validate_resource_budget(&connection).unwrap();
        assert_eq!(
            (before.transitions, before.preparations, before.scan_bytes),
            (after.transitions, after.preparations, after.scan_bytes)
        );
        assert!(!journal.is_halted().unwrap());
    }
}
