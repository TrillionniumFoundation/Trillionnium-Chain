...
latest_bench=$(ls -1t "$ROOT"/trillionnium-rust/run/bench/* 2>/dev/null | head -n 1 || true)
latest_health=$(ls -1t "$ROOT"/trillionnium-rust/run/health/* 2>/dev/null | head -n 1 || true)
cat > "$REPORT" <<EOF
