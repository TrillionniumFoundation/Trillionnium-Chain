...
use crate::{
    load_acked, now_ms, resolve_path_arg_from_env, resolve_tx_retry_policy, WORKER_EVENT_LOG_ENV,
    WORKER_PROGRESS_LOG_ENV,
};

#[path = "flush_submission.rs"]
mod flush_submission;

use self::flush_submission::{process_submission_record, FlushRecordOutcome};
...
pub(crate) fn handle_flush_submissions(
    ...
    for line in raw.lines().filter(|l| !l.trim().is_empty()) {
        let rec: crate::state::SubmissionRecord = serde_json::from_str(line)?;
        n += 1;
