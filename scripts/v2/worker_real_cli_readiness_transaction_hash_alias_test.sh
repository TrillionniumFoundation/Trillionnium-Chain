...
TMP_DIR="$(mktemp -d /tmp/trnm-readiness-alias.XXXXXX)"
...
if [[ "${1:-}" == "tx" && "${2:-}" == "query" ]]; then
  echo '{"transactionHash":"0xABCDEF0123456789","transaction_status":"confirmed"}'
  exit 0
fi
exit 2
EOF
...
if [[ "$rc" -ne 0 ]]; then
  echo "[FAIL] readiness script should accept transaction_hash / transactionHash aliases (rc=$rc)" >&2
...
echo "[OK] worker_real_cli_readiness_transaction_hash_alias_test passed"
