...
BL09 retirement-prep note: any retained `trnm-pouw` gate, receipt, timing, or authority reference in this board should be read as migration-era compatibility, retention, or provenance / audit evidence only. Once PoCO becomes the primary settlement path, these retained surfaces are not the default payout authority and do not re-authorize default work-unit payout paths.
