...
mod tests {
    ...
    #[test]
    fn ingress_overflow_rejects_before_decode_or_durable_handoff() {
        let temporary = tempfile::tempdir().unwrap();
        let mut host = G1ProcessHostV0::open(temporary.path()).unwrap();
        for (current, submitted, expected_error) in [
            (u64::MAX, u64::MAX, "generation.overflow"),
            (u64::MAX - 2, u64::MAX - 1, "generation.horizon"),
            (u64::MAX - 1, u64::MAX, "generation.horizon"),
            // The last representable three-block horizon reaches the decoder.
            (u64::MAX - 3, u64::MAX - 2, "ingress.hex"),
        ] {
            host.generation = current;
            let error = host.submit_hex(submitted, "").unwrap_err();
            assert_eq!(error.code_v0(), expected_error);
            assert_eq!(host.generation, current);
            assert!(host.queue.is_empty());
            assert_eq!(host.accepted, 0);
            assert_eq!(host.boundary.retained_rows().unwrap(), 0);
            assert_eq!(
                host.application
                    .confirmed_committed_head_v0()
                    .unwrap()
