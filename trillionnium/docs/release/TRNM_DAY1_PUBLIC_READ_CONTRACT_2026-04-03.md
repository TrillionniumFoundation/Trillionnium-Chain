...
2. 对外暴露若需要独立入口，只能引用 `trillionnium/docs/runbooks/explorer-service-scaffold.md` 中的 operator-facing static scaffold；
3. 部署形态优先采用 **loopback bind + reverse proxy**，并把 runtime knobs 固定在 `trillionnium/run/explorer-service/explorer-service.env`；
4. handoff / signoff 时必须同时保存：
   ...
   - 一次 `/index.json` 抓取结果
   - 若需要可复制的 ticket / 值班文本骨架，直接引用 `trillionnium/docs/release/TRNM_EXPLORER_SCAFFOLD_HANDOFF_TEMPLATE_2026-04-04.md`
...
- signoff note 必须同时引用 `trillionnium/docs/runbooks/explorer-service-scaffold.md`
- 若当前证据仍来自 scaffold bring-up / `capture_explorer_scaffold_handoff.sh` / `explorer_service_status.sh`，或仍保留 `deployment_evidence_scope=placeholder-only`、`service_mode=operator-facing-static-scaffold`，则只能使用 `trillionnium/docs/release/TRNM_EXPLORER_SCAFFOLD_HANDOFF_TEMPLATE_2026-04-04.md`
...
- 只有当 `deployment_evidence_scope=durable-read-service`、`service_mode=non-placeholder-durable-read-service`、6 个 durable-read anchors 都已有真实值，且 replay / restore / checkpoint / lag evidence 同时具备时，才允许切换到 `trillionnium/docs/release/TRNM_DURABLE_READ_SERVICE_HANDOFF_TEMPLATE_2026-04-04.md`
