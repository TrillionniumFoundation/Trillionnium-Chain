...
readonly -a NO_CARGO_EXPECTED_SUMMARIES=(
  ...
  'poco_g3_run_material_self_test=passed positives=2 negatives=33 validator_hosts=5 mac_observer=true ephemeral_keys=true pop=true public_workload=true ordinary_start_height=4 ordinal_height_mapping=true content_addressed=true application_private_keys=false builder_inode_pinned=true builder_path_substitution_rejected=true material_builder_validator_binary_distinct=true same_binary_fallback_rejected=true material_author_hash_bound=true material_author_runtime_deployed=false run_root_symlink_rejected=true generator_output_symlink_rejected=true public_bootstrap_bundle=true bootstrap_runtime_closed=false production_activation=false geo_wan=false'
  'poco_g3_network_smoke_fleet_test=passed positives=5 negatives=7 unique_json=true safe_remote_paths=true input_symlinks_rejected=true file_backed_process_io=true partial_cleanup=true remote_binary_hash=true observer_stage_cleanup_registered=true validator_run_completed=false g3_complete=false geo_wan=false'
