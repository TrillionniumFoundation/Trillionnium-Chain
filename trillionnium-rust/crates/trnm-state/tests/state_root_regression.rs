...
fn checkpoint_da_light_verifier_summary_exposes_canonical_surface_fields() {
    ...
    assert!(summary.contains("wal_committed_kind=commit-flag-u8"));
...
#[test]
fn checkpoint_evidence_surface_rejects_non_genesis_prev_hash_with_edge_whitespace_even_when_hashes_match() {
    let wal = WalMeta {
        height: 2,
        round: 0,
        proposal_hash: "proposal-2".into(),
        committed: true,
        state_root_hex: "ab".repeat(32),
        prev_hash_hex: Some(format!(" {} ", "cd".repeat(32))),
    };
    let checkpoint = CheckpointMeta {
        height: 2,
        state_root_hex: wal.state_root_hex.clone(),
    ...
    assert!(
        !checkpoint_evidence_surface_is_canonical(&checkpoint, &wal),
        "checkpoint evidence surfaces must reject non-genesis WAL prev_hash_hex values with edge whitespace even when state_root_hex and wal_entry_hash_hex otherwise match, so predecessor linkage cannot silently canonicalize trimmed digest surfaces"
    );
    assert!(
        checkpoint_da_light_verifier_summary(&checkpoint, &wal).is_none(),
        "DA/light-verifier summaries must fail closed when non-genesis WAL prev_hash_hex carries edge whitespace instead of a canonical digest"
    );
}
...
#[test]
fn checkpoint_evidence_surface_accepts_max_length_proposal_hash_when_hashes_match() {
    let wal = WalMeta {
        height: 1,
        round: 0,
    ...
    assert!(
        checkpoint_evidence_surface_is_canonical(&checkpoint, &wal),
        "checkpoint evidence surfaces should accept WAL proposal identities at the canonical 256-byte boundary when state_root_hex and wal_entry_hash_hex match canonical digests"
    );

    let summary = checkpoint_da_light_verifier_summary(&checkpoint, &wal)
        .expect("canonical max-length proposal identities should still emit a DA/light-verifier summary");
    assert!(summary.contains("wal_proposal_hash_bytes=256"));
    assert!(summary.contains("wal_proposal_hash_surface_canonical=true"));
}
...
#[test]
fn checkpoint_da_light_verifier_summary_fails_closed_on_zero_width_wal_state_root() {
    let canonical_root = "ab".repeat(32);
    let wal = WalMeta {
        height: 1,
        round: 0,
    ...
    assert!(
        checkpoint_da_light_verifier_summary(&checkpoint, &wal).is_none(),
        "DA/light-verifier summaries must fail closed on zero-width WAL state_root_hex surfaces even when checkpoint state_root_hex and wal_entry_hash_hex mirror the same bytes"
    );
}
