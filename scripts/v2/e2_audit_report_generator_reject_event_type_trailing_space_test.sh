...
cat >"$LOG" <<'LOGEOF'
2026-03-02T06:30:00Z [event] event_schema=v1 event_type="resolve " challenger_delta=7 treasury_delta=-7 note="quoted trailing-space token should be ignored"
2026-03-02T06:30:01Z [event] event_schema=v1 event_type=resolve challenger_delta=3 treasury_delta=-3 note="canonical resolve token"
LOGEOF
...
if summary["total_audit_events"] != 1:
    print('[FAIL] expected exactly one canonical audit event after trailing-space event_type filtering', file=sys.stderr)
    sys.exit(1)
...
print('[PASS] e2_audit_report_generator_reject_event_type_trailing_space_test')
PY
