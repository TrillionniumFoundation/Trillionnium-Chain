...
# This regression deliberately exercises the local failure-injection override.
# The production gate must continue to reject overrides when CI=true.
if CI=false WEB4_RELEASE_REQUIRED_GATES="scripts/v2/definitely_missing_gate.sh" "$GATE" >"$NEG_LOG" 2>&1; then
