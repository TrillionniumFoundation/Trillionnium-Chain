...
This specification binds the M09 boundary to its primary crates in `config/module-coverage-v1.toml`. It defines inputs, deterministic outputs, failure behavior, resource limits, and evidence required before promotion.
...
The SLO profile is the one registered for M09 in `config/module-coverage-v1.toml`; measurements must report p50/p95/p99 latency, throughput, queue depth, memory, CPU, and error classes under bounded and adversarial workloads. No benchmark result alone establishes production readiness.
