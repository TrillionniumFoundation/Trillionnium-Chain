...
- the node has no production Vote loop/effect driver/authenticated P2P/state
  ...
  and lockfile no longer reach the legacy application crate or external BFT engine;
...
`Cargo.lock` contains no foreign/external BFT engine/external application adapter package, and executable adapter
