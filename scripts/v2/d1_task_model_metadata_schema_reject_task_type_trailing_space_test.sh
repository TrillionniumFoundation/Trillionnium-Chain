...
cat >"$tmp_file" <<'JSON'
{
  ...
  "provenance": {
    ...
    "produced_at": "2026-03-02T04:30:00Z",
...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] expected non-zero exit for trailing-space task_type" >&2
  exit 1
fi

if ! grep -Fq "task_type does not match pattern ^\\S(?:.*\\S)?$" <<<"$output"; then
  echo "[FAIL] missing explicit edge-whitespace pattern error for task_type" >&2
  echo "$output" >&2
  exit 1
fi

echo "[PASS] D1 schema gate rejects task_type with trailing space"
