...
class WorkflowWiringTests(unittest.TestCase):
    ...
    def test_no_protocol_command_removed(self):
        job = self.protocol_job()
        # Preserve the actual original commands, not a magic total which can
        # pass after one valuable command is replaced by an unrelated one.
        required = set("""
trnm_gate bash ./scripts/ci/check_canonical_development_plan.sh
trnm_gate bash ./scripts/ci/check_poco_bft_mainline_truth.sh
trnm_gate python3 ./scripts/ci/check_required_protocol_contract_v1.py
trnm_gate python3 ./scripts/ci/check_poco_bft_v0_parameters.py
trnm_gate python3 ./scripts/ci/check_poco_bft_v0_wire_vectors.py
trnm_gate python3 ./scripts/ci/check_poco_bft_v0_wire_reference.py
trnm_gate python3 ./scripts/ci/check_poco_bft_v0_wire_semantic_reference.py
trnm_gate python3 ./scripts/ci/check_poco_bft_v0_anchor_finality_vectors.py
trnm_gate python3 ./scripts/ci/check_poco_bft_v0_ordered_roots.py
trnm_gate bash ./scripts/ci/check_poco_bft_v0_qc_tc_vectors.sh
trnm_gate bash ./scripts/ci/check_poco_bft_v0_handoff_vectors.sh
trnm_gate bash ./scripts/ci/check_poco_bft_v0_registry_reference.sh --self-test
trnm_gate bash ./scripts/ci/check_poco_bft_v0_registry.sh
trnm_gate bash ./scripts/ci/check_poco_bft_v0_logical_schema.sh
trnm_gate bash ./scripts/ci/check_poco_bft_v0_anchor_handoff_schema.sh
trnm_gate bash ./scripts/ci/check_poco_bft_v0_epoch_commitment_schema.sh
trnm_gate bash ./scripts/ci/check_poco_bft_v0_block_body_schema.sh
trnm_gate bash ./scripts/ci/check_poco_bft_v0_checkpoint_finality_schema.sh
trnm_gate bash ./scripts/ci/check_poco_bft_v0_joint_handoff_schema.sh
trnm_gate bash ./scripts/ci/check_poco_bft_v0_snapshot_candidate_schema.sh
trnm_gate bash ./scripts/ci/check_poco_bft_v0_consumption_certificate_schema.sh
trnm_gate bash ./scripts/ci/check_poco_bft_v0_snapshot_namespace_schema.sh
trnm_gate bash ./scripts/ci/check_poco_bft_v0_snapshot_transition_schema.sh
trnm_gate bash ./scripts/ci/check_poco_bft_v0_checkpoint_execution_schema.sh
trnm_gate bash ./scripts/ci/check_poco_bft_v0_business_semantics_schema.sh
trnm_gate bash ./scripts/ci/check_poco_bft_v0_application_authority_schema.sh
trnm_gate bash ./scripts/ci/check_poco_bft_v0_authenticated_candidate_selection.sh
trnm_gate bash ./scripts/ci/check_poco_bft_v0_authenticated_next_epoch_commitment.sh
trnm_gate bash ./scripts/ci/check_poco_bft_v0_authenticated_checkpoint_handoff.sh
""".strip().splitlines())
        actual = [line.strip() for line in job.splitlines()
                  if line.strip().startswith('trnm_gate ')]
        self.assertTrue(required <= set(actual), required - set(actual))
        self.assertEqual(len(actual), len(set(actual)))
        self.assertIn('trnm_gate node --test ./scripts/ci/test_checkpoint_pre_certificate_v0.mjs', actual)
