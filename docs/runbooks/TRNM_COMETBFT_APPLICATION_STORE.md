...
chain ID, app version 7, and the canonical authorized-signer policy. Startup
probes an existing database read-only and rejects an app-version-6 binding
before opening the writable store. It never rewrites that binding: preserve the
v6 database and use a reviewed, version-specific export/new-genesis ceremony
