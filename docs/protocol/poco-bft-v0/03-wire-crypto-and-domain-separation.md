...
This is bounded validation-parent authority, not a general host/external application adapter runtime-
...
continues to use its direct-read path and no external application adapter outcome consumes that view.
...
`AuthorizedNativeCheckpointExecutionV0`, checkpoint, Core, or external application adapter. A separate
...
terminal promotion, and host/Core/external application adapter callback wiring remain open. The Core transport holder now
...
terminal-result, vote, finality, checkpoint, or external application adapter authority.
...
effect driver, fresh execution, general result takeover, host/AppCore/external application adapter
...
terminal-result, vote, finality, checkpoint, or external application adapter authority. Non-runtime
...
versus-invariant host mapping, production host/Core callback wiring, external application adapter
...
Core `Input` or external application adapter operation. A narrow consuming bridge may prepare only the
...
crash takeover, external application adapter, the `Valid` validation-time and Finalize-time atomic
...
external application adapter/host integration also remain open.
Protocol v0 drivers MUST NOT invent failed receipts or choose a different
classification per implementation. external application adapter `ProcessProposal` has no faithful
