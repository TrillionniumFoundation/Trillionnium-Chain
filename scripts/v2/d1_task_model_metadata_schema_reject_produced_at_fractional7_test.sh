...
cat >"$tmp_file" <<'JSON'
{
  ...
  "provenance": {
    ...
    "produced_at": "2026-03-02T01:15:00.1234567Z",
...
if [[ "$rc" -eq 0 ]]; then
  echo "[FAIL] expected non-zero exit for produced_at with 7-digit fractional seconds" >&2
  exit 1
fi
...
echo "[PASS] D1 schema gate rejects produced_at with 7-digit fractional seconds"
