...
//! native header field using the authenticated native execution context.
...
//! activation, or a Core epoch transition.

use crate::{
    poco_checkpoint::AuthorizedNativeCheckpointExecutionV0,
...
/// or signed. Every field belongs to the native frozen header.
...
/// tuple. This capability binds the native [`BlockId`] and does not authorize
/// checkpoint finality or a handoff.
...
pub(crate) fn prepare_poco_checkpoint_header_v0(
    ...
    exact_parent_header
        ...
        native_execution.parent_height() == exact_parent_header.height()
            ...
            && native_execution.parent_id() == exact_parent_header.id()
            && native_execution.timestamp_ms() == checkpoint_timestamp_ms,
...
/// capability. The returned ID is exactly `BlockHeader::id()`.
...
/// Revalidate the existing bound reservation at the consumption boundary.
/// This cannot recreate a deleted slot: journal.bind requires the original
/// exact reservation, durable row, journal identity, and non-halted state.
pub(crate) fn revalidate_durably_bound_poco_checkpoint_header_v0(
    journal: &PocoPreparationJournalV0,
    bound: &DurablyBoundPocoCheckpointHeaderV0,
) -> Result<()> {
    let record = PocoCheckpointBoundReplayRecordV0::new(
        bound
            .authorized()
            .header()
            .try_cev0_bytes()
            .map_err(|e| anyhow::anyhow!("encode existing bound checkpoint: {e:?}"))?,
        bound.native_block_id(),
        bound.authorization_id(),
    )?;
    journal.bind(&bound._reservation, &record)
}
...
///
/// The native owner calls this after strict evidence validation and live
/// execution planning, before returning a bound checkpoint preparation.
