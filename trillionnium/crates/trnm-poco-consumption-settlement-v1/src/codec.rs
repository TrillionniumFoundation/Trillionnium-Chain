...
use crate::error::{error, ConsumptionSettlementErrorCodeV1, ConsumptionSettlementResultV1};

pub(crate) fn canonical_bytes<T: BorshSerialize>(
    value: &T,
...
pub(crate) fn strict_decode<T>(bytes: &[u8]) -> ConsumptionSettlementResultV1<T>
where
    T: BorshDeserialize + BorshSerialize,
{
    let value: T = borsh::from_slice(bytes).map_err(|cause| {
        error(
            ConsumptionSettlementErrorCodeV1::NonCanonical,
            cause.to_string(),
        )
...
pub(crate) fn checksum(parts: &[&[u8]]) -> Hash32V1 {
    let mut hasher = Sha256::new();
    hasher.update(b"trnm.poco-ai.consumption-settlement-store-checksum.candidate.v1");
    for part in parts {
